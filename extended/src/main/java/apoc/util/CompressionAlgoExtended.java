...
public enum CompressionAlgoExtended {
    NONE(null, null),
    ...
    CompressionAlgoExtended(Class<?> compressor, Class<?> decompressor) {
    ...
    public CountingInputStreamExtended toInputStream(byte[] data) {
        extendedApocConfig().isImportFileEnabled();

        try {
            ByteArrayInputStream stream = new ByteArrayInputStream(data);
            InputStream inputStream = toLimitedIStream(getInputStream(stream), data.length);
            return new CountingInputStreamExtended(inputStream, stream.available());
        } catch (Exception e) {
