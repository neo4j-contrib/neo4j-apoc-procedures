 ...
 * @since 26.01.18
 */
public interface StreamConnectionExtended {
    ...
    default CountingInputStreamExtended toCountingInputStream(String algo) throws IOException {
        InputStream iStream = toLimitedIStream(getInputStream(), getLength());

        if ("gzip".equals(getEncoding()) || getName().endsWith(".gz")) {
            return new CountingInputStreamExtended(new GZIPInputStream(iStream), getLength());
        }
        if ("deflate".equals(getName())) {
            return new CountingInputStreamExtended(new DeflaterInputStream(iStream), getLength());
        }
        try {
            final InputStream inputStream = CompressionAlgoExtended.valueOf(algo == null ? CompressionAlgoExtended.NONE.name() : algo)
                    .getInputStream(iStream);
            return new CountingInputStreamExtended(inputStream, getLength());
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }

    static class UrlStreamConnection implements StreamConnectionExtended {
    ...
    class FileStreamConnection implements StreamConnectionExtended {
