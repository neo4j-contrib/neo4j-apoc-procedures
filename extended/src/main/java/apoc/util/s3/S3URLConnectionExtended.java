...
import apoc.util.StreamConnectionExtended;
...
public class S3URLConnectionExtended extends URLConnection {
    ...
    public S3URLConnectionExtended(URL url) {
        super(url);
    }
    ...
    public static StreamConnectionExtended openS3InputStream(URL url) {
        S3ParamsExtended s3Params = S3ParamsExtractorExtended.extract(url);
        String region = Objects.nonNull(s3Params.getRegion()) ? s3Params.getRegion() : Regions.US_EAST_1.getName();
        return new S3AwsExtended(s3Params, region).getS3AwsInputStream(s3Params);
    }
}
