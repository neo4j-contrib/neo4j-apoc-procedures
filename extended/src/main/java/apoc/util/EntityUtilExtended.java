...
public class EntityUtilExtended {
    ...
    public static <T> T anyRebind(Transaction tx, T any) {
        ...
        if (any instanceof Path) {
            ...
            PathImpl.Builder builder = new PathImpl.Builder(UtilExtended.rebind(tx, path.startNode()));
            for (Relationship rel : path.relationships()) {
                builder = builder.push(UtilExtended.rebind(tx, rel));
            }
        ...
        if (any instanceof Entity) {
            return (T) UtilExtended.rebind(tx, (Entity) any);
        }
        return any;
    }
}
