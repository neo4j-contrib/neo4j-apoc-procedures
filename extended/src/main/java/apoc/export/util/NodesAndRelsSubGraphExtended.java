 ...
 * @since 22.05.16
 */
public class NodesAndRelsSubGraphExtended implements SubGraphExtended {
    ...
    private <T> ArrayList<T> getDefinitions(
        ...
        for (String label : labels) {
            IterablesExtended.addAll(definitions, nodeFunction.apply(schema, Label.label(label)));
        }
        for (String type : types) {
            IterablesExtended.addAll(definitions, relFunction.apply(schema, RelationshipType.withName(type)));
        }
        return definitions;
    }
    ...
    public Iterable<IndexDefinition> getIndexes(Label label) {
        ...
        return UtilExtended.getIndexes(tx, label);
    }
    ...
    public Iterable<IndexDefinition> getIndexes(RelationshipType type) {
        ...
        return UtilExtended.getIndexes(tx, type);
    }
