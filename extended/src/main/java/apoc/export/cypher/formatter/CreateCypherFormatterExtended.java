 ...
 * @since 16-06-2017
 */
public class CreateCypherFormatterExtended extends AbstractCypherFormatterExtended implements CypherFormatterExtended {
    ...
    public String statementForNode(
        ...
        String labels = CypherFormatterUtilsExtended.formatAllLabels(node, uniqueConstraints, indexNames);
        if (!labels.isEmpty()) {
            result.append(labels);
        }
        result.append(" {");
        result.append(CypherFormatterUtilsExtended.formatNodeProperties("", node, uniqueConstraints, indexNames, true));
        result.append("}");
        result.append(");");
    ...
    public String statementForRelationship(
        ...
        StringBuilder result = new StringBuilder(100);
        result.append("MATCH ");
        result.append(CypherFormatterUtilsExtended.formatNodeLookup(
                "n1", relationship.getStartNode(), uniqueConstraints, indexedProperties));
        result.append(", ");
        result.append(CypherFormatterUtilsExtended.formatNodeLookup(
                "n2", relationship.getEndNode(), uniqueConstraints, indexedProperties));
        result.append(" CREATE (n1)-[r:" + UtilExtended.quote(relationship.getType().name()));
        if (relationship.getPropertyKeys().iterator().hasNext()) {
            result.append(" {");
            result.append(CypherFormatterUtilsExtended.formatRelationshipProperties("", relationship, true));
            result.append("}");
        }
        result.append("]->(n2);");
        return result.toString();
