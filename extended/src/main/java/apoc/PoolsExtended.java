...
import apoc.periodic.PeriodicUtilsExtended;
...
public class PoolsExtended extends LifecycleAdapter {
    ...
    private final Map<PeriodicUtilsExtended.JobInfo, Future> jobList = new ConcurrentHashMap<>();

    public PoolsExtended(LogService log, GlobalProcedures globalProceduresRegistry, ExtendedApocConfig apocConfig) {

        this.log = log.getInternalLog(PoolsExtended.class);
        this.apocConfig = apocConfig;

        // expose this config instance via `@Context ApocConfig config`
        globalProceduresRegistry.registerComponent((Class<PoolsExtended>) getClass(), ctx -> this, true);
        this.log.info("successfully registered PoolsExtended for @Context");
    }
    ...
    public void init() {
        ...
        int threads =
                Math.max(1, apocConfig.getInt(ExtendedApocConfig.APOC_CONFIG_JOBS_POOL_NUM_THREADS, DEFAULT_POOL_THREADS));

        int queueSize = Math.max(1, apocConfig.getInt(ExtendedApocConfig.APOC_CONFIG_JOBS_QUEUE_SIZE, threads * 5));
        ...
        this.scheduledExecutorService = Executors.newScheduledThreadPool(
                Math.max(
                        1,
                        apocConfig.getInt(
                                ExtendedApocConfig.APOC_CONFIG_JOBS_SCHEDULED_NUM_THREADS, DEFAULT_SCHEDULED_THREADS)),
                threadFactory);

        scheduledExecutorService.scheduleAtFixedRate(
                () -> {
                    for (Iterator<Map.Entry<PeriodicUtilsExtended.JobInfo, Future>> it =
                                    jobList.entrySet().iterator();
                            it.hasNext(); ) {
                        Map.Entry<PeriodicUtilsExtended.JobInfo, Future> entry = it.next();
    ...
    public Map<PeriodicUtilsExtended.JobInfo, Future> getJobList() {
        return jobList;
    }
