...
public class PeriodicExtended {
    ...
    public Stream<PeriodicUtilsExtended.JobInfo> submitSchema(@Name("name") String name, @Name("statement") String statement, @Name(value = "params", defaultValue = "{}") Map<String,Object> config) {
        validateQuery(statement);
        return PeriodicUtilsExtended.submitProc(name, statement, config, db, log, pools);
    }
    
    private void validateQuery(String statement) {
        UtilExtended.validateQuery(db, statement,
