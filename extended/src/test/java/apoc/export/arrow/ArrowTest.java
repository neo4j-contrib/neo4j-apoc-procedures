...
public class ArrowTest {
    ...
    public void testStreamRoundtripArrowQuery() {
        ...
        final byte[] byteArray = db.executeTransactionally(
                "CYPHER 25 CALL apoc.export.arrow.stream.query($query) YIELD value AS byteArray",
    ...
    public void testStreamRoundtripArrowGraph() {
        // given - when
        final byte[] byteArray = db.executeTransactionally(
                "CYPHER 25 CALL apoc.graph.fromDB('neo4j',{}) yield graph "
    ...
    private void testStreamRoundtripAllCommon() {
        // given - when
        final byte[] byteArray = db.executeTransactionally(
                "CYPHER 25 CALL apoc.export.arrow.stream.all() YIELD value AS byteArray ", Map.of(), this::extractByteArray);
    ...
    public void testStreamVolumeArrowAll() {
        ...
        final List<byte[]> list = db.executeTransactionally(
                "CYPHER 25 CALL apoc.export.arrow.stream.query('MATCH (n:ArrowNode) RETURN n.id AS id') YIELD value AS byteArray ",
                Map.of(),
        ...
        // then
        final String query = "CYPHER 25 UNWIND $list AS byteArray " + "CALL apoc.load.arrow.stream(byteArray) YIELD value "
    ...
    public void testFileVolumeArrowAll() {
        ...
        // then
        final String query = "CYPHER 25 CALL apoc.load.arrow($file) YIELD value " + "RETURN value.id AS id";
        db.executeTransactionally(query, Map.of("file", file), result -> {
            final List<Long> actual =
                    result.stream().map(m -> (Long) m.get("id")).sorted().collect(Collectors.toList());
            assertEquals(expected, actual);
