...
public class CoreExtendedTest {
    ...
    public void checkForCoreAndExtended() {
        try {
            Neo4jContainerExtension neo4jContainer = createEnterpriseDB(
                            List.of(TestContainerUtil.ApocPackage.FULL), !TestUtil.isRunningInCI())
            ...
            int coreCount = session.run("CALL apoc.help('') YIELD core WHERE core = true RETURN count(*) AS count")
                    ...
                    .get("count")
                    .asInt();
            int extendedCount = session.run("CALL apoc.help('') YIELD core WHERE core = false RETURN count(*) AS count")
                    .peek()
                    .get("count")
                    .asInt();
    ...
    public void matchesSpreadsheet() {
        try {
            ...
            File apocCoreCsv = new File(
                    ClassLoader.getSystemResource("apoc-core-extended.csv").getFile());
            ...
            Result result = session.run(
                    "load csv with headers from 'file:///apoc-core-extended.csv' AS row RETURN row.Name as Name, row.Decision AS Decision");

            Map<String, String> spreadsheet = new HashMap<>();
            List<Record> list = result.list();
            for (Record record : list) {
                spreadsheet.put(
                        record.get("Name").asString(), record.get("Decision").asString());
            }
            ...
            List<Map.Entry<String, String>> different = spreadsheet.entrySet().stream()
                    .filter(entry -> actual.containsKey(entry.getKey())
                            && !actual.get(entry.getKey()).equals(entry.getValue()))
