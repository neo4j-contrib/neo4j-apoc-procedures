...
public class DataVirtualizationCatalogTestUtil {
    ...
    public static final String JDBC_SELECT_QUERY_WITH_PARAM =
            "SELECT * FROM country WHERE Name = $name AND param_with_question_mark = ? ";
    ...
    public static final Map<String, Object> VIRTUALIZE_JDBC_QUERY_PARAMS =
            map(NAME_KEY, VIRTUALIZE_JDBC_COUNTRY, "CODE2", CODE2, "head_of_state", HEAD_OF_STATE);
    public static final Map<String, Object> VIRTUALIZE_JDBC_QUERY_WRONG_PARAMS =
            map("foo", VIRTUALIZE_JDBC_COUNTRY, "bar", CODE2, "baz", HEAD_OF_STATE);
    ...
    public static final String VIRTUALIZE_JDBC_WITH_PARAMS_QUERY =
            "SELECT * FROM country WHERE Name = $name AND HeadOfState = $head_of_state AND Code2 = $CODE2";
    public static final String APOC_DV_JDBC_WITH_PARAMS_QUERY =
    ...
    public static final Map<String, Object> APOC_DV_INSTALL_PARAMS = map(
            DATABASE_NAME,
            DEFAULT_DATABASE_NAME,
            NAME_KEY,
            CSV_NAME_VALUE,
            MAP_KEY,
            map("type", "CSV", "url", CSV_TEST_FILE, "query", QUERY_VALUE, "desc", DESC_VALUE, "labels", LABELS));
    ...
    public static final Map<String, Object> APOC_DV_DROP_PARAMS =
            map(DATABASE_NAME, DEFAULT_DATABASE_NAME, NAME_KEY, CSV_NAME_VALUE);
    ...
    public static Map<String, Object> getVirtualizeJDBCParameterMap(JdbcDatabaseContainer mysql, String query) {
        ...
        return map(
                ...
                JDBC_DESC,
                LABELS_KEY,
                JDBC_LABELS_AS_STRING);
    }

    public static void getVirtualizeCSVCommonResult(
            GraphDatabaseService dbRead, String dvAdd, String dvList, String csvUrl, GraphDatabaseService dbWrite) {
        ...
        testCall(
                ...
                map(
                        ...
                        CSV_NAME_VALUE,
                        MAP_KEY,
                        map("type", "CSV", "url", csvUrl, "query", QUERY_VALUE, "desc", DESC_VALUE, "labels", LABELS)),
                (row) -> assertCatalogContent(row, csvUrl));

        testCall(dbWrite, dvList, (row) -> assertCatalogContent(row, csvUrl));
    ...
    public static void getVirtualizeJDBCCommonResult(
            GraphDatabaseService db, JdbcDatabaseContainer mysql, String dvAdd, GraphDatabaseService dbWrite) {
        ...
        testCall(
                ...
                map(
                        ...
                        JDBC_NAME,
                        MAP_KEY,
                        getVirtualizeJDBCParameterMap(mysql, VIRTUALIZE_JDBC_QUERY)),
                (row) -> assertDvQueryContent(row, url));

        testCallCount(
                db,
                APOC_DV_QUERY_WITH_PARAM,
                map(NAME_KEY, JDBC_NAME, CONFIG_KEY, map(CREDENTIALS_KEY, getJdbcCredentials(mysql))),
                0);
    ...
    public static void getVirtualizeJDBCWithParamsCommonResult(
            GraphDatabaseService db, JdbcDatabaseContainer mysql, String dvAdd, GraphDatabaseService dbWrite)
        ...
        testCallEmpty(
                db,
                APOC_DV_JDBC_WITH_PARAMS_QUERY,
                map(NAME_KEY, JDBC_NAME, CONFIG_KEY, map(CREDENTIALS_KEY, getJdbcCredentials(mysql))));
