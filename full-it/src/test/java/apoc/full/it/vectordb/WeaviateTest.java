...
public class WeaviateTest {
    ...
    public static void setUp() throws Exception {
        ...
        testCall(
                db,
                "CALL apoc.vectordb.weaviate.delete($host, 'TestCollection', "
                ...
                r -> {
                    ...
                    assertEquals(
                            List.of("7ef2b3a7-1e56-4ddd-b8c3-2ca8901ce308", "7ef2b3a7-1e56-4ddd-b8c3-2ca8901ce309"),
                            value);
                });
    }
    ...
    public void getReadOnlyVectorsWithMapping() {
        Map<String, Object> conf = MapUtil.map(ALL_RESULTS_KEY, true, MAPPING_KEY, MapUtil.map(EMBEDDING_KEY, "vect"));

        try {
            testCall(
                    db,
                    "CALL apoc.vectordb.weaviate.get($host, 'TestCollection', [1, 2], $conf)",
                    map("host", HOST, "conf", conf),
                    r -> fail());
    ...
    public void queryReadOnlyVectorsWithMapping() {
        ...
        try {
            testCall(
                    db,
                    "CALL apoc.vectordb.weaviate.query($host, 'TestCollection', [0.2, 0.1, 0.9, 0.7], {}, 5, $conf)",
