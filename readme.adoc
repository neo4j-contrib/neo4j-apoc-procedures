...
APOC Extended can be used with the https://hub.docker.com/_/neo4j/[Neo4j Docker image] via the `NEO4JLABS_PLUGINS` environment variable.
...
./gradlew --init-script init.gradle shadow
...
A full build including running the tests can be run by `./gradlew --init-script init.gradle build`.
