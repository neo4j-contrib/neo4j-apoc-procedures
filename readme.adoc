...
Generate undirected (random direction) graphs with semi-real random distributions based on theoretical models. Providing a node label will generate
nodes with that label and a random uuid. Providing a relationship type will generate relationships with that type.
Providing `null` in place of labels and/or relationships will generate random `Person` nodes with a random `name` and
`FRIEND_OF` relationships, respectively.
