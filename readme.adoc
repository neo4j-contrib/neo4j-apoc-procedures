...
Utilities to find nodes with normal indexes in parallel. These procedures return a single list of nodes or a 'reduced' node information with the node id, labels, and the properties where the search was executed upon. Typically the reduced versions are used when the nodes have a lot of data/properties.

[cols="1m,5"]
|===
| call apoc.search.node(LabelPropertyMap, searchType: EXACT \| CONTAINS \| STARTS WITH \| ENDS WITH, searchValue ) yield node as <identifier> | For every label property defined in the LabelPropertyMap a search will be done as parallel as possible (depending on available cores). A distinct set of Nodes will be returned.
| call apoc.search.nodeAll(LabelPropertyMap, searchType: EXACT \| CONTAINS \| STARTS WITH \| ENDS WITH, searchValue ) yield node as <identifier> | For every label property defined in the LabelPropertyMap a search will be done as parallel as possible (depending on available cores). All the found Nodes will be returned.
| call apoc.search.nodeReduced(LabelPropertyMap, searchType: EXACT \| CONTAINS \| STARTS WITH \| ENDS WITH, searchValue ) yield id as <identifier>, labels as <identifier>, values as <identifier> | For every label property defined in the LabelPropertyMap a search will be done as parallel as possible (depending on available cores). A merged set of 'reduced' Node information will be returned.
| call apoc.search.nodeAll(LabelPropertyMap, searchType: EXACT \| CONTAINS \| STARTS WITH \| ENDS WITH, searchValue ) yield node as <identifier> | For every label property defined in the LabelPropertyMap a search will be done as parallel as possible (depending on available cores). All the found 'reduced' Node information will be returned.
|===

example:
[source,cypher]
----
call apoc.search.node('{Address :["address"],Entity: ["name","address","original_name"], Intermediary : "name", Officer :"name"}',"CONTAINS","Cameron") yield node as n return n
----
