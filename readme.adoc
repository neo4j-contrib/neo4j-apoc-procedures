...
* `CALL apoc.meta.type(value)`  - type name of a value
...
* `apoc.coll.contains(coll, value)` optimized contains operation (using a HashSet)
* `apoc.coll.containsSorted(coll, value)` optimized contains on a sorted list operation (Collections.binarySearch)
