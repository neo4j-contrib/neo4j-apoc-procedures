...
[cols="1m,5"]
|===
| apoc.index.search('name', 'query') YIELD node, weight | search for nodes in the given full text index matching the given lucene query
...
[cols="1m,5"]
|===
| CALL apoc.warmup.run() |  quickly loads all nodes and rels into memory by skipping one page at a time
|===
