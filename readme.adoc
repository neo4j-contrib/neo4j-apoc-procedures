...
Typically the reduced versions are used when the nodes contain a lot of data/properties. To have the most benefit of a fast search make sure you use properties which are indexed. The search will be executed as parallel as possible depending on the amount of available cores.

[cols="1m,5"]
|===
| call apoc.search.node(LabelPropertyMap, searchType: EXACT \| CONTAINS \| STARTS WITH \| ENDS WITH, searchValue ) yield node as <identifier> | A distinct set of Nodes will be returned.
| call apoc.search.nodeAll(LabelPropertyMap, searchType: EXACT \| CONTAINS \| STARTS WITH \| ENDS WITH, searchValue ) yield node as <identifier> | All the found Nodes will be returned.
| call apoc.search.nodeReduced(LabelPropertyMap, searchType: EXACT \| CONTAINS \| STARTS WITH \| ENDS WITH, searchValue ) yield id as <identifier>, labels as <identifier>, values as <identifier> | A merged set of 'reduced' Node information will be returned. Per node one record.
| call apoc.search.nodeAllReduced(LabelPropertyMap, searchType: EXACT \| CONTAINS \| STARTS WITH \| ENDS WITH, searchValue ) yield id as <identifier>, labels as <identifier>, values as <identifier> | All the found 'reduced' Node information will be returned.
|===


LabelPropertyMap

The label property map can have multiple labels and one or more properties per label. 

Syntax: { label1 : "propertyOne", label2 :["propOne","propTwo"] }

For every Label-property combination a parallel search will be executed: Label1.propertyOne, label2.propOne and label2.propTwo
