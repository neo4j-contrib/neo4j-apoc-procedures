...
public class ExportJsonS3Test extends S3BaseTest {
    ...
    @Rule
    public DbmsRule db = new ImpermanentDbmsRule().withSetting(ApocSettings.apoc_export_file_enabled, true);
    ...
    public void testExportAllJson() throws Exception {
        ...
        TestUtil.testCall(db, "CALL apoc.export.json.all($s3,null)", map("s3", s3Url), (r) -> {
    ...
    public void testExportPointMapDatetimeJson() throws Exception {
        ...
        String s3Url = s3Container.getUrl(filename);
        String query =
                "return {data: 1, value: {age: 12, name:'Mike', data: {number: [1,3,5], born: date('2018-10-29'), place: point({latitude: 13.1, longitude: 33.46789})}}} as map, "
    ...
    public void testExportGraphJson() throws Exception {
        ...
        TestUtil.testCall(
                db,
                "CALL apoc.graph.fromDB('test',{}) yield graph " + "CALL apoc.export.json.graph(graph, $s3) "
