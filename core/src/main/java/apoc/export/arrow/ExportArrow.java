...
public class ExportArrow {
    ...
    @Description(
            "apoc.export.arrow.stream.graph(graph, config) - exports given nodes and relationships as arrow byte[] result")
    ...
    @Description(
            "apoc.export.arrow.stream.query(query, config) - exports results from the cypher statement as arrow byte[] result")
    ...
    public Stream<ProgressInfo> all(
            @Name("fileName") String fileName, @Name(value = "config", defaultValue = "{}") Map<String, Object> config)
            throws Exception {
        return new ExportArrowService(db, pools, terminationGuard, logger)
    ...
    @Description(
            "apoc.export.arrow.graph(fileName, graph, config) - exports given nodes and relationships as arrow file")
    ...
    @Description(
            "apoc.export.arrow.query(fileName, query, config) - exports results from the cypher statement as arrow file")
