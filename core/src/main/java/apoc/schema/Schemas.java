...
public class Schemas {
    ...
    public Boolean indexExistsOnRelationship(
            @Name("labelName") String relName, @Name("propertyName") List<String> propertyNames) {
    ...
    public List<AssertSchemaResult> assertConstraints(Map<String, List<Object>> constraints0, boolean dropExisting)
        ...
        for (ConstraintDefinition definition : schema.getConstraints()) {
            ...
            String label = isRelConstraint(definition)
    ...
    private AssertSchemaResult createNodeKeyConstraint(String lbl, List<Object> keys) {
        ...
        tx.execute(String.format(
                        "CREATE CONSTRAINT ON (n:`%s`) ASSERT (%s) IS NODE KEY", Util.sanitize(lbl), keyProperties))
                .close();
    ...
    private AssertSchemaResult createCompoundIndex(String label, List<String> keys) {
        ...
        tx.execute(String.format("CREATE INDEX ON :`%s` (%s)", Util.sanitize(label), String.join(",", backTickedKeys)))
                .close();
    ...
    private Stream<IndexConstraintNodeInfo> indexesAndConstraintsForNode(Map<String, Object> config) {
        ...
        try (Statement ignore = ktx.acquireStatement()) {
            ...
            final Predicate<ConstraintDefinition> isNodeConstraint =
                    constraintDefinition -> !isRelConstraint(constraintDefinition);
