...
public class DatabaseSubGraph implements SubGraph {
    ...
    public long countsForRelationship(Label start, RelationshipType type, Label end) {
        // even if `MATCH ()-[r]->(:Label) RETURN count(r)` and `MATCH (:Label)-[r]->() RETURN count(r)` leverage on
        // `RelationshipCountFromCountStore`
        // given that with this method there is planner overhead, we count entities via the `TokenRead`, because it's
        // much faster
        ...
        if (transaction instanceof InternalTransaction) {
            ...
            return kernelTransaction.dataRead().countsForRelationship(startId, relId, endId);
        }
    ...
    public long countsForNode(Label label) {
        ...
        if (transaction instanceof InternalTransaction) {
            ...
            return kernelTransaction.dataRead().countsForNode(labelId);
        }
        return transaction
