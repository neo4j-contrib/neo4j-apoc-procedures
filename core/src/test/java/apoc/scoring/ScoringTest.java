...
public class ScoringTest {
    ...
    public void existence() throws Exception {
        TestUtil.testCall(
                db, "RETURN apoc.scoring.existence(10,true) as score", (row) -> assertEquals(10D, row.get("score")));
        TestUtil.testCall(
                db, "RETURN apoc.scoring.existence(10,false) as score", (row) -> assertEquals(0D, row.get("score")));
    }
    ...
    public void pareto() throws Exception {
        TestUtil.testResult(
                db,
                "UNWIND [0,1,2,8,10,100] as value RETURN value, apoc.scoring.pareto(2,8,10,value) as score",
                (r) -> {
