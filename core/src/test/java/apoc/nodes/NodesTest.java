...
public class NodesTest {
    ...
    public void types() throws Exception {
        ...
        TestUtil.testCall(
                db, "RETURN apoc.node.relationship.types(null) AS value", (r) -> assertEquals(null, r.get("value")));
    }
    ...
    public void nodesTypes() throws Exception {
        ...
        // when
        TestUtil.testCall(db, "MATCH (n) RETURN apoc.nodes.relationship.types(collect(n)) AS value", (result) -> {
            // then
    ...
    public void nodesHasRelationship() throws Exception {
        ...
        // when
        TestUtil.testCall(
                db, "MATCH (n:Foo) RETURN apoc.nodes.relationships.exist(collect(n), 'X|Y') AS value", (result) -> {
                    // then
                    List<Map<String, Object>> list = (List<Map<String, Object>>) result.get("value");
                    assertFalse("value should not be empty", list.isEmpty());
                    list.forEach(map -> {
