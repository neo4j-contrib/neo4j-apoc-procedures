...
public class XmlTest {
    ...
    public void testLoadXml() {
        testCall(db, "CALL apoc.load.xml('file:databases.xml')", (row) -> {
    ...
    public void testBookIds() {
        testResult(
                db,
                "call apoc.load.xml('" + TestUtil.getUrlFileName("xml/books.xml") + "') yield value as catalog\n"
    ...
    public void testReturnCollectionElements() {
        testResult(
                ...
                "call apoc.load.xml('" + TestUtil.getUrlFileName("xml/books.xml") + "') yield value as catalog "
                        + "unwind catalog._children as book "
                        + "return book.id as id, [x in book._children where x._type = 'author' | x._text] as authors, [x in book._children where x._type='title' | x._text] as title",
                result -> {
                    assertEquals(
                            "+-----------------------------------------------------------------------------------------------------+\n"
                                    + "| id      | authors                                      | title                                      |\n"
    ...
    public void testLoadXmlXpathAuthorFromBookId() {
        testCall(
                db,
                "CALL apoc.load.xml('" + TestUtil.getUrlFileName("xml/books.xml")
                        + "', '/catalog/book[@id=\"bk102\"]/author') yield value as result",
                (r) -> {
    ...
    public void testLoadXmlXpathGenreFromBookTitle() {
        testCall(
                db,
                "CALL apoc.load.xml('" + TestUtil.getUrlFileName("xml/books.xml")
                        + "', '/catalog/book[title=\"Maeve Ascendant\"]/genre') yield value as result",
                (r) -> {
    ...
    public void testLoadXmlXpathReturnBookFromBookTitle() {
        testCall(
                db,
                "CALL apoc.load.xml('" + TestUtil.getUrlFileName("xml/books.xml")
                        + "', '/catalog/book[title=\"Maeve Ascendant\"]/.') yield value as result",
                (r) -> {
    ...
    public void testLoadXmlXpathBooKsFromGenre() {
        testResult(
                db,
                "CALL apoc.load.xml('" + TestUtil.getUrlFileName("xml/books.xml")
                        + "', '/catalog/book[genre=\"Computer\"]') yield value as result",
                (r) -> {
    ...
    public void testLoadXmlWithNextEntityRels() {
        testCall(
                db,
                "call apoc.xml.import('file:" + FILE_SHORTENED + "', "
    ...
    public void testLoadXmlFromZip() {
        testResult(
                db,
                "call apoc.load.xml('file:src/test/resources/testload.zip!xml/books.xml') yield value as catalog\n"
                        + "UNWIND catalog._children as book\n"
                        + "RETURN book.id as id\n",
                result -> {
                    List<Object> ids = Iterators.asList(result.columnAs("id"));
    ...
    public void testLoadXmlFromTar() {
        testResult(
                db,
                "call apoc.load.xml('file:src/test/resources/testload.tar!xml/books.xml') yield value as catalog\n"
                        + "UNWIND catalog._children as book\n"
                        + "RETURN book.id as id\n",
                result -> {
                    List<Object> ids = Iterators.asList(result.columnAs("id"));
    ...
    public void testLoadXmlFromTarGz() {
        testResult(
                db,
                "call apoc.load.xml('file:src/test/resources/testload.tar.gz!xml/books.xml') yield value as catalog\n"
                        + "UNWIND catalog._children as book\n"
                        + "RETURN book.id as id\n",
                result -> {
                    List<Object> ids = Iterators.asList(result.columnAs("id"));
    ...
    public void testLoadXmlFromTgz() {
        testResult(
                db,
                "call apoc.load.xml('file:src/test/resources/testload.tgz!xml/books.xml') yield value as catalog\n"
                        + "UNWIND catalog._children as book\n"
                        + "RETURN book.id as id\n",
                result -> {
                    List<Object> ids = Iterators.asList(result.columnAs("id"));
    ...
    public void testLoadXmlFromZipByUrl() {
        testResult(
                db,
                "call apoc.load.xml('http://localhost:6363/testload.zip?raw=true!xml/books.xml') yield value as catalog\n"
                        + "UNWIND catalog._children as book\n"
                        + "RETURN book.id as id\n",
                result -> {
                    List<Object> ids = Iterators.asList(result.columnAs("id"));
    ...
    public void testLoadXmlFromTarByUrl() {
        testResult(
                db,
                "call apoc.load.xml('http://localhost:6363/testload.tar.gz?raw=true!xml/books.xml') yield value as catalog\n"
                        + "UNWIND catalog._children as book\n"
                        + "RETURN book.id as id\n",
                result -> {
                    List<Object> ids = Iterators.asList(result.columnAs("id"));
    ...
    public void testLoadXmlFromTgzByUrl() {
        testResult(
                db,
                "call apoc.load.xml('http://localhost:6363/testload.tgz?raw=true!xml/books.xml') yield value as catalog\n"
                        + "UNWIND catalog._children as book\n"
                        + "RETURN book.id as id\n",
                result -> {
                    List<Object> ids = Iterators.asList(result.columnAs("id"));
    ...
    public void testLoadXmlSingleLine() {
        testCall(
                db,
                "CALL apoc.load.xml('" + TestUtil.getUrlFileName("xml/singleLine.xml")
    ...
    public void testParseWithXPath() throws Exception {
        String xmlString =
                FileUtils.readFileToString(new File("src/test/resources/xml/books.xml"), Charset.forName("UTF-8"));
        testCall(
                db,
