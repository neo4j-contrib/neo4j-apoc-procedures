...
public class SchemasEnterpriseFeaturesTest {
    ...
    public void testDropConstraintExistsPropertyNode() {
        session.writeTransaction(tx -> {
            tx.run("CREATE CONSTRAINT FOR (m:Movie) REQUIRE exists(m.title)");
            tx.commit();
            return null;
        });
    ...
    public void testDropConstraintExistsPropertyRelationship() {
        session.writeTransaction(tx -> {
            tx.run("CREATE CONSTRAINT FOR ()-[acted:Acted]->() REQUIRE exists(acted.since)");
            tx.commit();
            return null;
        });
    ...
    public void testPropertyExistenceConstraintOnNode() {
        session.writeTransaction(tx -> {
            tx.run("CREATE CONSTRAINT FOR (bar:Bar) REQUIRE exists(bar.foobar)");
            return null;
        });
    ...
    public void testConstraintExistsOnRelationship() {
        session.writeTransaction(tx -> {
            tx.run("CREATE CONSTRAINT FOR ()-[like:LIKED]-() REQUIRE exists(like.day)");
            tx.commit();
            return null;
        });
