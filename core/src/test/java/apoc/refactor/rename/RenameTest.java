...
public class RenameTest {
    ...
    public void testRenameLabelForSomeNodes() throws Exception {
        ...
        testCall(
                db,
                "CALL apoc.refactor.rename.label($oldName,$newName, $nodes)",
                map("oldName", "Foo", "newName", "Bar", "nodes", nodes.subList(0, 3)),
                (r) -> {});
    ...
    public void testRenameLabel() throws Exception {
        ...
        testCall(
                db,
                "CALL apoc.refactor.rename.label($oldName,$newName)",
                map("oldName", "Foo", "newName", "Bar"),
                (r) -> {});
    ...
    public void testRenameLabelDoesntAllowCypherInjection() throws Exception {
        ...
        testCall(
                db,
                "CALL apoc.refactor.rename.label(" + "  'Foo', "
        ...
        testCall(
                db,
                "CALL apoc.refactor.rename.label(" + "  'Whatever` WITH n MATCH (m) DETACH DELETE m //', "
                        + "  'Foo'"
                        + ")",
                map(),
                (r) -> {});
    ...
    public void testRenameRelationship() throws Exception {
        ...
        testCall(
                db,
                "CALL apoc.refactor.rename.type($oldType,$newType)",
                map("oldType", "KNOWS", "newType", "LOVES"),
                (r) -> {});
    ...
    public void testRenameTypeForSomeRelationships() throws Exception {
        ...
        testCall(
                ...
                map("oldType", "KNOWS", "newType", "LOVES", "rels", rels),
                (r) -> {});
    ...
    public void testRenameTypeDoesntAllowCypherInjection() throws Exception {
        ...
        testCall(
                db,
                "CALL apoc.refactor.rename.type(" + "  'KNOWS', "
                        + "  'Whatever` WITH n MATCH (m) DETACH DELETE m //'"
                        + ")",
                map(),
                (r) -> {});
        ...
        testCall(
                db,
                "CALL apoc.refactor.rename.type(" + "  'Whatever` WITH n MATCH (m) DETACH DELETE m //', "
                        + "  'KNOWS'"
                        + ")",
                map(),
                (r) -> {});
    ...
    public void testRenameNodesProperty() throws Exception {
        ...
        testCall(
                ...
                map("oldName", "name", "newName", "surname"),
                (r) -> {});
    ...
    public void testRenamePropertyForSomeNodes() throws Exception {
        ...
        testCall(
                ...
                map("oldName", "name", "newName", "surname", "nodes", nodes.subList(0, 3)),
                (r) -> {});
    ...
    public void testRenamePropertyDoesntAllowCypherInjection() throws Exception {
        ...
        testCall(
                db,
                "CALL apoc.refactor.rename.nodeProperty(" + "  'name', "
                        + "  'Whatever` WITH n MATCH (m) DETACH DELETE m //'"
                        + ")",
                map(),
                (r) -> {});
        ...
        testCall(
                db,
                "CALL apoc.refactor.rename.nodeProperty(" + "  'Whatever` WITH n MATCH (m) DETACH DELETE m //', "
                        + "  'name'"
                        + ")",
                map(),
                (r) -> {});
    ...
    public void testRenameTypeProperty() throws Exception {
        ...
        testCall(
                db,
                "CALL apoc.refactor.rename.typeProperty($oldName,$newName)",
                map("oldName", "name", "newName", "surname"),
                (r) -> {});
    ...
    public void testRenameTypePropertyDoesntAllowCypherInjection() throws Exception {
        ...
        testCall(
                db,
                "CALL apoc.refactor.rename.typeProperty(" + "  'name', "
        ...
        testCall(
                db,
                "CALL apoc.refactor.rename.typeProperty(" + "  'Whatever ` = null remove r.name //', "
                        + "  'name'"
                        + ")",
                map(),
                (r) -> {});
    ...
    public void testLockException() {
        ...
        assertEventually(
                () -> db.executeTransactionally(
                        ...
                        r -> r.<String>columnAs("currentQuery").stream().anyMatch(curr -> curr.contains(lockQuery))),
                (v) -> v,
                20,
                TimeUnit.SECONDS);
        ...
        testResult(db, testQuery, Collections.emptyMap(), (r) -> {
            final Map<String, Object> batch =
                    r.<Map<String, Object>>columnAs("batch").next();
    ...
    public void testRenamePropertyForSomeRelationship() throws Exception {
        ...
        testCall(
                ...
                map("oldName", "name", "newName", "surname", "rels", rels),
                (r) -> {});
    ...
    public void testRenameWithSameValues() {
        ...
        testCall(
                db,
                "CALL apoc.refactor.rename.type('REL_TO_RENAME', 'REL_TO_RENAME')",
                r -> assertEquals(1L, r.get("total")));
    ...
    private long resultRelationshipsMatches(String type, String prop) {
        String query = type != null
                ? "MATCH ()-[r:`" + type + "`]->() RETURN count(r) as countResult"
                : "match ()-[r]->() where exists (r.`" + prop + "`) return count(r) as countResult";
    ...
    private long resultNodesMatches(String label, String prop) {
        String query = label != null
                ? "MATCH (b:`" + label + "`) RETURN count(b) as countResult"
                : "match (n) where exists (n.`" + prop + "`) return count(n) as countResult";
        return TestUtil.singleResultFirstColumn(db, query);
    }
}
