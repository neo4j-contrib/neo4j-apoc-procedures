...
public class GraphRefactoringTest {
    ...
    public void testDeleteOneNode() throws Exception {
        long id = db.executeTransactionally(
                "CREATE (p1:Person {ID:1}), (p2:Person {ID:2}) RETURN id(p1) as id ",
                emptyMap(),
                result -> Iterators.single(result.columnAs("id")));
        testCall(
                db,
                "MATCH (o:Person {ID:$oldID}), (n:Person {ID:$newID}) DELETE o RETURN n as node",
    ...
    public void testMergeNodesShouldNotFailWithSamePropKeysConstraints() {
        ...
        long id = db.executeTransactionally(
                "CREATE (a:A {prop1: 1}), (:B {prop1: 1}) RETURN id(a) as id",
                emptyMap(),
    ...
    public void testMergeNodesIndexConflict() throws Exception {
        ...
        db.executeTransactionally("CALL db.awaitIndexes()");
        long id = db.executeTransactionally(
                "CREATE (a:A) SET a.prop1 = 1 CREATE (b:B) SET b.prop2 = 99 RETURN id(a) as id ",
                emptyMap(),
                result -> Iterators.single(result.columnAs("id")));
        testCall(
                db,
    ...
    public void testExtractNode() throws Exception {
        ...
        testCall(
                db,
                "CALL apoc.refactor.extractNode($ids,['FooBar'],'FOO','BAR')",
                map("ids", singletonList(id)),
                (r) -> {
    ...
    public void testCollapseNode() throws Exception {
        Long id = db.executeTransactionally(
                "CREATE (f:Foo)-[:FOO {a:1}]->(b:Bar {c:3})-[:BAR {b:2}]->(f) RETURN id(b) as id",
                emptyMap(),
                result -> Iterators.single(result.columnAs("id")));
        testCall(db, "CALL apoc.refactor.collapseNode($ids,'FOOBAR')", map("ids", singletonList(id)), (r) -> {
            assertEquals(id, r.get("input"));
            Relationship rel = (Relationship) r.get("output");
    ...
    public void testMergeNodesWithConstraints() throws Exception {
        ...
        long id = db.executeTransactionally(
                "CREATE (p1:Person {name:'Foo'}), (p2:Person {surname:'Bar'}) RETURN id(p1) as id",
                emptyMap(),
                result -> Iterators.single(result.columnAs("id")));
        testCall(
                db,
    ...
    public void testMergeNodesWithIngoingRelationships() throws Exception {
        long lisaId = db.executeTransactionally(
                "CREATE \n" + "(alice:Person {name:'Alice'}),\n"
                        ...
                        + "(alice)-[:knows]->(bob),\n"
                        + "(lisa)-[:knows]->(alice),\n"
                        + "(bob)-[:knows]->(john) return id(lisa) as lisaId",
                emptyMap(),
    ...
    public void testMergeNodesWithSelfRelationships() throws Exception {
        ...
        testCall(
                ...
                (r) -> {
                    ...
                    assertTrue(node.getSingleRelationship(RelationshipType.withName("likes"), Direction.OUTGOING)
                            .getEndNode()
                            .equals(node));
                });
    }
    ...
    public void testMergeRelsOverwriteEagerAggregation() throws Exception {
        ...
        testCall(
                ...
                "MATCH (d:Person {name:'Daniele'})\n" + "MATCH (p:Country {name:'USA'})\n"
                        ...
                        + "return p,d,u,u.to as to, count(u) as totRel",
                (r) -> {
    ...
    public void testMergeNodesOnArrayValues() throws Exception {
        long id = db.executeTransactionally(
                "CREATE (p1:Person {ID:1, prop: ['foo']}), (p2:Person {ID:2, prop: ['foo']}) RETURN id(p1) as id ",
                emptyMap(),
                result -> Iterators.single(result.columnAs("id")));
        testCall(
                db,
