...
public class GraphRefactoringTest {
    ...
    public void deleteAndReconnect() throws Exception {
        ...
        TestUtil.testFail(db, "MATCH p=(f:One)-->(b:Two)-->(c:Three), (d:Four), (e:Five) WITH p, [d,e] as list CALL apoc.refactor.deleteAndReconnect(p, list) YIELD nodes, relationships RETURN nodes, relationships", RuntimeException.class);
        ...
        // - terminal node
        TestUtil.testCall(db, "MATCH p=(f:One)-->(c:Three)-->(e:Five) WITH p, f CALL apoc.refactor.deleteAndReconnect(p, [f]) YIELD nodes, relationships RETURN nodes, relationships",
                (row) -> {
                    List<Node> nodes = (List<Node>) row.get("nodes");
