...
public class PhoneticTest {
    ...
    public void shouldComputeSimpleSoundexEncoding() {
        testCall(
                db,
                "CALL apoc.text.phonetic('HellodearUser!')",
    ...
    public void shouldComputeSimpleSoundexEncodingOfNull() {
        testCall(db, "CALL apoc.text.phonetic(null)", (row) -> assertThat(row.get("value"), equalTo(null)));
    ...
    public void shouldComputeEmptySoundexEncodingForTheEmptyString() {
        testCall(db, "CALL apoc.text.phonetic('')", (row) -> assertThat(row.get("value"), equalTo("")));
    ...
    public void shouldComputeSoundexEncodingOfManyWordsEvenIfTheStringContainsSomeExtraChars() {
        testCall(
                db,
                "CALL apoc.text.phonetic('  ,Hello,  dear User 5!')",
                (row) -> assertThat(row.get("value"), equalTo("H400D600U260")));
        testCall(
                db,
    ...
    public void shoudlComputeDoubleMetaphone() {
        testCall(db, "CALL apoc.text.doubleMetaphone('Apoc')", (row) -> assertThat(row.get("value"), equalTo("APK")));
        testCall(
                db,
    ...
    public void shoudlComputeDoubleMetaphoneOfNull() {
        testCall(db, "CALL apoc.text.doubleMetaphone(NULL)", (row) -> assertThat(row.get("value"), equalTo(null)));
        testCall(
                db,
    ...
    public void shoudlComputeDoubleMetaphoneForTheEmptyString() {
        testCall(db, "CALL apoc.text.doubleMetaphone('')", (row) -> assertThat(row.get("value"), equalTo("")));
        testCall(
                db,
    ...
    public void shouldComputeDoubleMetaphoneOfManyWords() {
        testCall(
                db,
                "CALL apoc.text.doubleMetaphone('Hello, dear User!')",
