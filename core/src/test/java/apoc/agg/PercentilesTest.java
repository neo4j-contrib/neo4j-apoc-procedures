...
public class PercentilesTest {
    ...
    public void testPercentiles() throws Exception {
        ...
        testCall(db, "UNWIND [0,1,1,2,2,2,3] as value RETURN apoc.agg.percentiles(value,[0.5,0.95]) as p", (row) -> {
            assertEquals(asList(2L, 3L), row.get("p"));
        });
        testCall(db, "UNWIND [1,1,1,1,2,2,3,4] as value RETURN apoc.agg.percentiles(value) as p", (row) -> {
            assertEquals(asList(1L, 2L, 3L, 4L, 4L), row.get("p"));
        });
        testCall(
                db,
                "UNWIND [1,1,1,1,2,2,3,4] as value RETURN apoc.agg.percentiles(value,[0.5,0.75,0.9,0.95,0.99]) as p",
                (row) -> {
                    assertEquals(asList(1L, 2L, 3L, 4L, 4L), row.get("p"));
                });
    }
    ...
    public void testPercentilesDoubles() throws Exception {
        ...
        testCall(db, "UNWIND [0,1,1,2.0,2,2,3] as value RETURN apoc.agg.percentiles(value,[0.5,0.95]) as p", (row) -> {
            assertSameValues(asList(2D, 3D), row.get("p"));
        });
        testCall(db, "UNWIND [1,1,1.0,1,2,2,3,4] as value RETURN apoc.agg.percentiles(value) as p", (row) -> {
            assertSameValues(asList(1D, 2D, 3D, 4D, 4D), row.get("p"));
        });
        testCall(
                db,
                "UNWIND [1,1,1,1,2,2,3,4.0] as value RETURN apoc.agg.percentiles(value,[0.5,0.75,0.9,0.95,0.99]) as p",
                (row) -> {
                    assertSameValues(asList(1D, 2D, 3D, 4D, 4D), row.get("p"));
                });
    }
