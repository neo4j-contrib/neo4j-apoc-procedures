...
public class CollAggregationTest {
    ...
    public void testFirst() throws Exception {
        testCall(db, "UNWIND RANGE(0,10) as value RETURN apoc.agg.first(value) as first", (row) -> {
            assertEquals(0L, row.get("first"));
        });
        testCall(db, "UNWIND [null,42,43,null] as value RETURN apoc.agg.first(value) as first", (row) -> {
    ...
    public void testLast() throws Exception {
        testCall(db, "UNWIND RANGE(0,10) as value RETURN apoc.agg.last(value) as last", (row) -> {
            assertEquals(10L, row.get("last"));
        });
        testCall(db, "UNWIND [null,41,null,42,null] as value RETURN apoc.agg.last(value) as last", (row) -> {
    ...
    public void testSlice() throws Exception {
        testCall(db, "UNWIND RANGE(0,10) as value RETURN apoc.agg.slice(value,1,3) as slice", (row) -> {
            assertEquals(asList(1L, 2L, 3L), row.get("slice"));
        });
        testCall(db, "UNWIND RANGE(0,3) as value RETURN apoc.agg.slice(value) as slice", (row) -> {
            assertEquals(asList(0L, 1L, 2L, 3L), row.get("slice"));
        });
        testCall(db, "UNWIND RANGE(0,3) as value RETURN apoc.agg.slice(value,2) as slice", (row) -> {
