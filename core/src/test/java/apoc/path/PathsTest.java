...
public class PathsTest {
    ...
    public void createEmpty() throws Exception {
        ...
        TestUtil.testCall(
                db, "MATCH (a:A) RETURN apoc.path.create(a,null) as p", (row) -> assertPath(row, 0, "A", "A"));
        TestUtil.testCall(
                db,
                "MATCH (a:A)-[r:NEXT]->() RETURN apoc.path.create(a,[r]) as p",
                (row) -> assertPath(row, 1, "A", "B"));
    }
    ...
    public void slice() throws Exception {
        TestUtil.testCall(
                ...
                (row) -> assertPath(row, 1, "B", "C"));
        TestUtil.testCall(
                db,
                "MATCH path = (a:A)-[:NEXT*]->(d:D) RETURN apoc.path.slice(path,1) as p",
                (row) -> assertPath(row, 2, "B", "D"));
        TestUtil.testCall(
                db,
                "MATCH path = (a:A)-[:NEXT*]->(d:D) RETURN apoc.path.slice(path,1,100) as p",
                (row) -> assertPath(row, 2, "B", "D"));
        TestUtil.testCall(
                db,
                "MATCH path = (a:A)-[:NEXT*]->(d:D) RETURN apoc.path.slice(path,-1,0) as p",
                (row) -> assertPath(row, 0, "A", "A"));
        TestUtil.testCall(
                db,
                "MATCH path = (a:A)-[:NEXT*]->(d:D) RETURN apoc.path.slice(path,1,0) as p",
                (row) -> assertPath(row, 0, "B", "B"));
    ...
    public void combineFail() throws Exception {
        TestUtil.testCall(
                db,
                "MATCH p1 = (a:A)-[:NEXT]->(b:B), p2 = (c:C)-[:NEXT]->() RETURN apoc.path.combine(p1,p2) AS p",
    ...
    public void combine() throws Exception {
        TestUtil.testCall(
                ...
                (row) -> assertPath(row, 2, "A", "C"));
        TestUtil.testCall(
                db,
                "MATCH p1 = (a:A)-[:NEXT]->(b:B) RETURN apoc.path.combine(p1,null) as p",
                (row) -> assertPath(row, 1, "A", "B"));
        TestUtil.testCall(
                db,
                "MATCH p1 = (a:A)-[:NEXT]->(b:B) RETURN apoc.path.combine(null,p1) as p",
                (row) -> assertPath(row, 1, "A", "B"));
    }
