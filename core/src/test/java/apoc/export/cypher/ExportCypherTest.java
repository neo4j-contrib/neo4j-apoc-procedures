...
public class ExportCypherTest {
    ...
    private static final String exportDataWithRelOnly =
            "MATCH (start:Foo)-[rel:KNOWS]->(end:Bar) " + "CALL apoc.export.cypher.data([], [rel], $file, $config) "
                    + "YIELD nodes, relationships, properties RETURN *";
    ...
    public DbmsRule db = new ImpermanentDbmsRule()
            ...
            .withSetting(
                    newBuilder("unsupported.dbms.debug.track_cursor_close", BOOL, false)
                            .build(),
                    false)
            .withSetting(
                    newBuilder("unsupported.dbms.debug.trace_cursors", BOOL, false)
                            .build(),
                    false);
    ...
    public void testExportAllCypherSchemaWithSaveIdxNames() throws Exception {
        ...
        TestUtil.testCall(
                ...
                (r) -> assertResults(fileName, r, "database"));
        final String expectedFile = String.format(
                EXPECTED_SCHEMA_WITH_NAMES, " barIndex", " fooIndex", StringUtils.EMPTY, StringUtils.EMPTY);
    ...
    public void testExportAllCypherSchemaWithSaveConstraintNames() throws Exception {
        ...
        TestUtil.testCall(
                ...
                (r) -> assertResults(fileName, r, "database"));
        final String expectedFile = String.format(
                EXPECTED_SCHEMA_WITH_NAMES,
                StringUtils.EMPTY,
                StringUtils.EMPTY,
                " consBar",
                " uniqueConstraintComposite");
        assertEquals(expectedFile, readFile("all.schema.cypher"));
    }
    ...
    public void exportMultiTokenIndex() {
        ...
        db.executeTransactionally(
                "CALL db.index.fulltext.createNodeIndex('MyCoolNodeFulltextIndex',['TempNode', 'TempNode2'],['value', 'value2'])");
    ...
    public void shouldSaveCorrectlyRelIndexesWithNameOptimized() throws FileNotFoundException {
        ...
        relIndexTestCommon(
                ...
                map("ifNotExists", false, "saveIndexNames", true, "saveConstraintNames", true));

        // with ifNotExists: true
        relIndexTestCommon(
                fileName,
                EXPECTED_SCHEMA_OPTIMIZED_WITH_RELS_IF_NOT_EXISTS_AND_NAME,
                map("ifNotExists", true, "saveIndexNames", true, "saveConstraintNames", true));
    ...
    public void shouldSaveCorrectlyTypeIndexes() {
        ...
        String expectedSchema = String.format("BEGIN%n" + EXPECTED_BAR_FOO_INDEXES
    ...
    public static class ExportCypherResults {
        ...
        public static final String EXPECTED_SCHEMA = String.format(
                "BEGIN%n" + EXPECTED_BAR_FOO_INDEXES + EXPECTED_BAR_UNIQUE_CONSTRAINTS + "COMMIT%n" + "SCHEMA AWAIT%n");

        static final String EXPECTED_SCHEMA_WITH_NAMES =
                "BEGIN%n" + "CREATE BTREE INDEX%s FOR (node:Bar) ON (node.first_name, node.last_name);%n"
        ...
        public static final String EXPECTED_ONLY_SCHEMA_NEO4J_SHELL =
                String.format("BEGIN%n" + "CREATE BTREE INDEX FOR (node:Bar) ON (node.first_name, node.last_name);%n"
                        + "CREATE BTREE INDEX FOR (node:Foo) ON (node.name);%n"
                        + "CREATE CONSTRAINT ON (node:Bar) ASSERT (node.name) IS UNIQUE;%n"
                        + "CREATE CONSTRAINT ON (node:Bar) ASSERT (node.name, node.age) IS UNIQUE;%n"
                        + "COMMIT%n"
        ...
        static final String EXPECTED_SCHEMA_WITH_RELS_AND_NAME_OPTIMIZED = String.format(
                "BEGIN%n" + "CREATE BTREE INDEX barIndex FOR (node:Bar) ON (node.first_name, node.last_name);%n"
        ...
        static final String EXPECTED_SCHEMA_WITH_RELS_AND_IF_NOT_EXISTS = String.format(
                "BEGIN%n" + "CREATE BTREE INDEX IF NOT EXISTS FOR (node:Bar) ON (node.first_name, node.last_name);%n"
        ...
        static final String EXPECTED_SCHEMA_OPTIMIZED =
                String.format("BEGIN%n" + "CREATE BTREE INDEX FOR (node:Bar) ON (node.first_name, node.last_name);%n"
                        + "CREATE BTREE INDEX FOR (node:Foo) ON (node.name);%n"
                        + "CREATE CONSTRAINT ON (node:Bar) ASSERT (node.name) IS UNIQUE;%n"
                        + "CREATE CONSTRAINT ON (node:Bar) ASSERT (node.name, node.age) IS UNIQUE;%n"
        ...
        static final String EXPECTED_NODES_OPTIMIZED_BATCH_SIZE_UNWIND = String.format(
                "BEGIN%n" + "UNWIND [{_id:0, properties:{born:date('2018-10-31'), name:\"foo\"}}] AS row%n"
                        + "CREATE (n:`UNIQUE IMPORT LABEL`{`UNIQUE IMPORT ID`: row._id}) SET n += row.properties SET n:Foo;%n"
        ...
        static final String EXPECTED_NODES_OPTIMIZED_PARAMS_BATCH_SIZE_ODD = String.format(":begin%n"
        ...
        public static final String EXPECTED_UPDATE_ALL_UNWIND = String.format(
                "CREATE BTREE INDEX FOR (node:Bar) ON (node.first_name, node.last_name);%n"
                        + "CREATE BTREE INDEX FOR (node:Foo) ON (node.name);%n"
                        + "CREATE CONSTRAINT ON (node:Bar) ASSERT (node.name) IS UNIQUE;%n"
                        + "CREATE CONSTRAINT ON (node:Bar) ASSERT (node.name, node.age) IS UNIQUE;%n"
        ...
        public static final String EXPECTED_NEO4J_OPTIMIZED = EXPECTED_SCHEMA_OPTIMIZED
                + EXPECTED_NODES_OPTIMIZED
                + EXPECTED_RELATIONSHIPS_OPTIMIZED
                + DROP_UNIQUE_OPTIMIZED;

        public static final String EXPECTED_NEO4J_OPTIMIZED_BATCH_SIZE = EXPECTED_SCHEMA_OPTIMIZED
                + EXPECTED_NODES_OPTIMIZED_BATCH_SIZE
                + EXPECTED_RELATIONSHIPS_OPTIMIZED
                + DROP_UNIQUE_OPTIMIZED;

        static final String EXPECTED_NEO4J_SHELL_OPTIMIZED = EXPECTED_SCHEMA_OPTIMIZED
                + EXPECTED_NODES_OPTIMIZED
                + EXPECTED_RELATIONSHIPS_OPTIMIZED
                + DROP_UNIQUE_OPTIMIZED;

        static final String EXPECTED_NEO4J_SHELL_OPTIMIZED_BATCH_SIZE = EXPECTED_SCHEMA_OPTIMIZED
                + EXPECTED_NODES_OPTIMIZED_BATCH_SIZE
                + EXPECTED_RELATIONSHIPS_OPTIMIZED
                + DROP_UNIQUE_OPTIMIZED;

        static final String EXPECTED_QUERY_NODES = EXPECTED_SCHEMA_OPTIMIZED
                + EXPECTED_QUERY_NODES_OPTIMIZED
                + EXPECTED_RELATIONSHIPS_OPTIMIZED
                + DROP_UNIQUE_OPTIMIZED;
        static final String EXPECTED_QUERY_NODES2 = EXPECTED_SCHEMA_OPTIMIZED
        ...
        static final String EXPECTED_CYPHER_OPTIMIZED_BATCH_SIZE_UNWIND = EXPECTED_SCHEMA_OPTIMIZED
                + EXPECTED_NODES_OPTIMIZED_BATCH_SIZE_UNWIND
                + EXPECTED_RELATIONSHIPS_OPTIMIZED
                + DROP_UNIQUE_OPTIMIZED_BATCH;

        static final String EXPECTED_CYPHER_OPTIMIZED_BATCH_SIZE_ODD = EXPECTED_SCHEMA_OPTIMIZED
                + EXPECTED_NODES_OPTIMIZED_BATCH_SIZE_ODD
                + EXPECTED_RELATIONSHIPS_ODD
                + DROP_UNIQUE_OPTIMIZED_BATCH;

        static final String EXPECTED_CYPHER_SHELL_PARAMS_OPTIMIZED_ODD = EXPECTED_SCHEMA_OPTIMIZED
        ...
        public static final String EXPECTED_QUERY_CYPHER_SHELL_OPTIMIZED_UNWIND =
                EXPECTED_CYPHER_OPTIMIZED_BATCH_SIZE_UNWIND
        ...
        public static final String EXPECTED_QUERY_CYPHER_SHELL_PARAMS_OPTIMIZED_ODD =
                EXPECTED_CYPHER_SHELL_PARAMS_OPTIMIZED_ODD
                        .replace(NEO4J_SHELL.begin(), CYPHER_SHELL.begin())
                        .replace(NEO4J_SHELL.commit(), CYPHER_SHELL.commit())
                        .replace(NEO4J_SHELL.schemaAwait(), EXPECTED_INDEXES_AWAIT)
                        .replace(NEO4J_SHELL.schemaAwait(), CYPHER_SHELL.schemaAwait());
        ...
        public static final String EXPECTED_CYPHER_SHELL_OPTIMIZED_BATCH_SIZE =
                EXPECTED_NEO4J_SHELL_OPTIMIZED_BATCH_SIZE
                        .replace(NEO4J_SHELL.begin(), CYPHER_SHELL.begin())
                        .replace(NEO4J_SHELL.commit(), CYPHER_SHELL.commit())
                        .replace(NEO4J_SHELL.schemaAwait(), EXPECTED_INDEXES_AWAIT)
                        .replace(NEO4J_SHELL.schemaAwait(), CYPHER_SHELL.schemaAwait());
        ...
        public static final String EXPECTED_ONLY_SCHEMA_CYPHER_SHELL = EXPECTED_ONLY_SCHEMA_NEO4J_SHELL
                        .replace(NEO4J_SHELL.begin(), CYPHER_SHELL.begin())
                        .replace(NEO4J_SHELL.commit(), CYPHER_SHELL.commit())
                        .replace(NEO4J_SHELL.schemaAwait(), CYPHER_SHELL.schemaAwait())
        ...
        public static final List<String> EXPECTED_CONSTRAINTS = List.of(
                ...
                "CREATE CONSTRAINT ON (node:Label2) ASSERT (node.prop) IS NOT NULL;",
                "CREATE CONSTRAINT ON ()-[rel:TYPE2]-() ASSERT (rel.prop) IS NOT NULL;",
                "CREATE CONSTRAINT ON (node:Label3) ASSERT (node.prop) IS NODE KEY;",
        ...
        public static final String EXPECTED_CYPHER_SHELL_WITH_COMPOUND_CONSTRAINT = String.format(
                "CREATE CONSTRAINT ON (node:Person) ASSERT (node.name, node.surname) IS NODE KEY;%n" + ":commit%n"
