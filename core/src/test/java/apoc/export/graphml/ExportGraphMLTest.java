...
public class ExportGraphMLTest {
    ...
    private static final String exportDataWithRelOnly =
            "MATCH (start:Start)-[rel:REL]->(end:End) " + "CALL apoc.export.graphml.data([], [rel], $file, {}) "
    ...
    public void testImportGraphML() throws Exception {
        ...
        fw.write(EXPECTED_TYPES);
        fw.close();
        TestUtil.testCall(
                db, "CALL apoc.import.graphml($file,{readLabels:true})", map("file", output.getAbsolutePath()), (r) -> {
    ...
    public void testRoundtripInvalidUnicodeWithExportQuery() {
        ...
        final String query =
                "CALL apoc.export.graphml.query('MATCH (n:Unicode) RETURN n', $file, {useTypes: true, readLabels:true})";
    ...
    private void testRoundtripInvalidUnicodeCommon(String query, String file) {
        ...
        TestUtil.testCall(db, query, map("file", file), (r) -> assertEquals(1L, r.get("nodes")));
    ...
    public void testImportInvalidUnicodeFile() {
        final String file =
                ClassLoader.getSystemResource("fileWithUnicode.graphml").toString();
        testImportInvalidUnicode(file);
    }

    private void testImportInvalidUnicode(String file) {
        TestUtil.testCall(db, "CALL apoc.import.graphml($file, {readLabels:true})", map("file", file), r -> {
    ...
    public void testRoundTripWithSeparatedImport() {
        ...
        Map<String, Object> importConfig = map(
                "readLabels", true, "storeNodeIds", true, "source", map("label", "Foo"), "target", map("label", "Bar"));
    ...
    public void testImportSeparatedFilesWithCustomId() {
        Map<String, Object> exportConfig =
                map("useTypes", true, "source", map("id", "name"), "target", map("id", "age"));

        Map<String, Object> importConfig = map(
                "readLabels",
                true,
                "source",
                map("label", "Foo", "id", "name"),
                "target",
                map("label", "Bar", "id", "age"));
    ...
    private void separatedFileCommons(Map<String, Object> exportConfig, Map<String, Object> importConfig) {
        db.executeTransactionally(
                "CREATE (:Foo {name: 'zzz'})-[:KNOWS]->(:Bar {age: 0}), (:Foo {name: 'aaa'})-[:KNOWS {id: 1}]->(:Bar {age: 666})");
        ...
        TestUtil.testCall(
                db,
                "CALL apoc.export.graphml.query('MATCH (start:Foo)-[:KNOWS]->(:Bar) RETURN start',$file, $config)",
                map("file", outputNodesFoo, "config", exportConfig),
                (r) -> assertEquals(3L, r.get("nodes")));

        TestUtil.testCall(
                db,
                "CALL apoc.export.graphml.query('MATCH (:Foo)-[:KNOWS]->(end:Bar) RETURN end', $file, $config) ",
                map("file", outputNodesBar, "config", exportConfig),
                (r) -> assertEquals(3L, r.get("nodes")));

        TestUtil.testCall(
                db,
                "MATCH (:Foo)-[rel:KNOWS]->(:Bar) WITH collect(rel) as rels \n"
        ...
        TestUtil.testCall(
                db,
                "CALL apoc.import.graphml($file, $config)",
    ...
    public void testImportGraphMLTerminate() {
        final String file = ClassLoader.getSystemResource("largeFile.graphml").toString();
        checkTerminationGuard(db, 5L, "CALL apoc.import.graphml($file,{readLabels:true})", map("file", file));
    }
    ...
    public void testImportGraphMLWithEdgeWithoutDataKeys() throws Exception {
        ...
        File output = new File(directory, "import.graphml");
        FileWriter fw = new FileWriter(output);
        fw.write(EXPECTED_TYPES_NO_DATA_KEY);
        fw.close();
        TestUtil.testCall(
                db,
        ...
        TestUtil.testCall(
                db, "MATCH  ()-[c:RELATED]->() RETURN COUNT(c) AS c", null, (r) -> assertEquals(1L, r.get("c")));
    }
    ...
    public void issue2797WithImportGraphMl() {
        ...
        try {
            ...
            String expected =
                    "Failed to invoke procedure `apoc.import.graphml`: Caused by: IndexEntryConflictException{propertyValues=( String(\"foo\") ), addedNodeId=-1, existingNodeId=3}";
        ...
        TestUtil.testCall(
                db,
                "MATCH (n:FOO) RETURN properties(n) AS props",
    ...
    public void testImportGraphMLWithoutCharactersDataKeys() throws Exception {
        ...
        File output = new File(directory, "import.graphml");
        FileWriter fw = new FileWriter(output);
        fw.write(EXPECTED_TYPES_WITHOUT_CHAR_DATA_KEYS);
        fw.close();
        TestUtil.testCall(
                db, "CALL apoc.import.graphml($file,{readLabels:true})", map("file", output.getAbsolutePath()), (r) -> {
                    assertResults(output, r, "statement");
    ...
    public void testImportDefaultRelationship() throws Exception {
        ...
        FileWriter fw = new FileWriter(output);
        fw.write(NO_REL_TYPES);
        fw.close();
        db.executeTransactionally(
                "CALL apoc.import.graphml( $file, { batchSize: 5000, readLabels: true, defaultRelationshipType:'DEFAULT_TYPE' })",
                map("file", output.getAbsolutePath()));
        TestUtil.testCall(db, "MATCH ()-[r]-() RETURN Distinct type(r) as type", (r) -> {
    ...
    public void testImportGraphMLNodeEdge() throws Exception {
        ...
        FileWriter fw = new FileWriter(output);
        fw.write(EXPECTED_READ_NODE_EDGE);
        fw.close();
    ...
    public void testImportGraphMLNodeEdgeWithBinary() {
        ...
        commonAssertionImportNodeEdge(
                null,
                "CALL apoc.import.graphml($file,{readLabels:true, compression: 'DEFLATE'})",
    ...
    public void testExportAllGraphML() {
        ...
        TestUtil.testCall(
                db,
                "CALL apoc.export.graphml.all($file,null)",
                map("file", output.getAbsolutePath()),
                (r) -> assertResults(output, r, "database"));
    ...
    public void testExportAllGraphMLWithCompression() {
        ...
        TestUtil.testCall(
                db,
                "CALL apoc.export.graphml.all($file, $config)",
    ...
    public void testGraphMlRoundtrip() {
        ...
        final Map<String, Object> params = map(
                ...
                "config",
                map(CompressionConfig.COMPRESSION, algo.name(), "readLabels", true, "useTypes", true));
        TestUtil.testCall(
                db,
                "CALL apoc.export.graphml.all($file, $config)",
                params,
        ...
        TestUtil.testCall(
                db, "CALL apoc.import.graphml($file, $config) ", params, r -> assertEquals(3L, r.get("nodes")));
    ...
    public void testExportGraphGraphML() {
        ...
        TestUtil.testCall(
                db,
                "CALL apoc.graph.fromDB('test',{}) yield graph " + "CALL apoc.export.graphml.graph(graph, $file,null) "
    ...
    public void testImportAndExport() throws Exception {
        ...
        fw.write(EXPECTED_TYPES_EMPTY);
        fw.close();
        TestUtil.testCall(
                db,
                "CALL apoc.import.graphml($file,{readLabels:true})",
                map("file", input.getAbsolutePath()),
        ...
        db.executeTransactionally("MATCH (n) detach delete (n)");
        db.executeTransactionally(
                "CREATE (f:Foo:Foo2:Foo0 {name:'foo'})-[:KNOWS]->(b:Bar {name:'bar',age:42}),(c:Bar {age:12,values:[1,2,3]})");
    }
    ...
    private void assertResultEmpty(File output, Map<String, Object> r) {
        ...
        if (r.get("source").toString().contains(":")) assertEquals("database: nodes(1), rels(0)", r.get("source"));
    ...
    public void testExportGraphGraphMLQueryGephi() {
        ...
        TestUtil.testCall(
                db,
                "call apoc.export.graphml.query('MATCH p=()-[r]->() RETURN p limit 1000',$file,{useTypes:true, format: 'gephi'}) ",
                map("file", output.getAbsolutePath()),
                (r) -> {
    ...
    public void testExportStartAndRelWithNodesOfRelsTrue() {
        ...
        commonDataAndQueryAssertions(exportQueryStartAndRel, params, EXPECTED_START_AND_REL_QUERY, 2L, 1L, 3L);
    ...
    private void assertExportWithoutEndNode(String exportQuery, Map<String, Object> params) {
        ...
        commonDataAndQueryAssertions(exportQuery, params, expectedWithoutEndNode, 1L, 1L, 2L);
    }
    ...
    private void assertExportRelOnly(String exportQuery, Map<String, Object> config) {
        ...
        commonDataAndQueryAssertions(exportQuery, config, expectedWithoutNodes, 0L, 1L, 1L);
    }
    ...
    public void testExportGraphGraphMLQueryGephiWithArrayCaption() {
        ...
        TestUtil.testCall(
                db,
                "call apoc.export.graphml.query('MATCH p=()-[r]->() RETURN p limit 1000',$file,{useTypes:true, format: 'gephi', caption: ['bar','name','foo']}) ",
                map("file", output.getAbsolutePath()),
                (r) -> {
                    assertEquals(2L, r.get("nodes"));
                    assertEquals(1L, r.get("relationships"));
    ...
    public void testExportGraphGraphMLQueryGephiWithArrayCaptionWrong() {
        ...
        TestUtil.testCall(
                db,
                "call apoc.export.graphml.query('MATCH p=()-[r]->() RETURN p limit 1000',$file,{useTypes:true, format: 'gephi', caption: ['c','d','e']}) ",
                map("file", output.getAbsolutePath()),
                (r) -> {
                    assertEquals(2L, r.get("nodes"));
                    assertEquals(1L, r.get("relationships"));
    ...
    public void testExportAllGraphMLTinker() {
        ...
        TestUtil.testCall(
                db,
                "CALL apoc.export.graphml.all($file, {format:'tinkerpop'})",
                map("file", output.getAbsolutePath()),
                (r) -> assertResults(output, r, "database"));
    ...
    public void testExportGraphGraphMLQueryTinkerPop() {
        ...
        TestUtil.testCall(
                db,
                "call apoc.export.graphml.query('MATCH p=()-[r]->() RETURN p limit 1000',$file,{useTypes:true, format: 'tinkerpop'}) ",
                map("file", output.getAbsolutePath()),
                (r) -> {
                    assertEquals(2L, r.get("nodes"));
                    assertEquals(1L, r.get("relationships"));
    ...
    public void testExportGraphGraphMLQueryTinkerPopWithArrayCaption() {
        ...
        TestUtil.testCall(
                db,
                "call apoc.export.graphml.query('MATCH p=()-[r]->() RETURN p limit 1000',$file,{useTypes:true, format: 'tinkerpop', caption: ['bar','name','foo']}) ",
                map("file", output.getAbsolutePath()),
                (r) -> {
                    assertEquals(2L, r.get("nodes"));
                    assertEquals(1L, r.get("relationships"));
    ...
    public void testExportGraphGraphMLQueryTinkerPopWithArrayCaptionWrong() {
        ...
        TestUtil.testCall(
                db,
                "call apoc.export.graphml.query('MATCH p=()-[r]->() RETURN p limit 1000',$file,{useTypes:true, format: 'tinkerpop', caption: ['c','d','e']}) ",
                map("file", output.getAbsolutePath()),
                (r) -> {
                    assertEquals(2L, r.get("nodes"));
                    assertEquals(1L, r.get("relationships"));
    ...
    public void testImportGraphmlPreventXXEVulnerabilityThrowsQueryExecutionException() {
        QueryExecutionException e = assertThrows(
                QueryExecutionException.class,
                () -> testResult(
                        db, "CALL apoc.import.graphml('" + TestUtil.getUrlFileName("xml/xxe.xml") + "', {})", (r) -> {
                            r.next();
                            r.close();
                        }));
    ...
    public void testImportGraphmlPreventBillionLaughVulnerabilityThrowsQueryExecutionException() {
        QueryExecutionException e = assertThrows(
                ...
                () -> testResult(
                        db,
                        "CALL apoc.import.graphml('" + TestUtil.getUrlFileName("xml/billion_laughs.xml") + "', {})",
                        (r) -> {
                            r.next();
                            r.close();
                        }));
    ...
    public void testExportAllGraphMLStream() {
        TestUtil.testCall(db, "CALL apoc.export.graphml.all(null, {stream: true})", (r) -> {
    ...
    public void testExportGraphmlAdminOperationErrorMessage() {
        ...
        for (String query : invalidQueries) {
            QueryExecutionException e = Assert.assertThrows(
                    ...
                    () -> TestUtil.testCall(
                            db,
                            "" + "CALL apoc.export.graphml.query(" + "$query," + "$file," + "{}" + ")",
