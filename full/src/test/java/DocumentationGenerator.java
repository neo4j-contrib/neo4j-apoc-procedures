...
class DocumentationGenerator {
    ...
    public void writeNavAndOverview() {
        ...
        String baseUrl = String.format("https://raw.githubusercontent.com/neo4j-contrib/neo4j-apoc-procedures/%s/docs/asciidoc/modules/ROOT/pages/",
        ...
        try (Writer overviewWriter = new OutputStreamWriter(new FileOutputStream(new File(DocsTest.GENERATED_PARTIALS_DOCUMENTATION_DIR, "documentation.adoc")), StandardCharsets.UTF_8);
             ...
             Writer coreWriter = new OutputStreamWriter(new FileOutputStream(new File(DocsTest.GENERATED_PARTIALS_DOCUMENTATION_DIR, "core.adoc")), StandardCharsets.UTF_8);
             Writer fullWriter = new OutputStreamWriter(new FileOutputStream(new File(DocsTest.GENERATED_PARTIALS_DOCUMENTATION_DIR, "full.adoc")), StandardCharsets.UTF_8)) {
            writeAutoGeneratedHeader(overviewWriter);
            writeAutoGeneratedHeader(navWriter);
            writeAutoGeneratedHeader(coreWriter);
            writeAutoGeneratedHeader(fullWriter);

            coreWriter.write(coreFullHeader(CORE));
            fullWriter.write(coreFullHeader(FULL));
            ...
            topLevelNamespaces.keySet().stream().sorted().forEach(topLevelNamespace -> {
                ...
                try {
                    ...
                    for (Row row : topLevelNamespaces.get(topLevelNamespace)) {
                        final boolean isFull = extended.contains(row.name);
                        String releaseType = isFull ? FULL : CORE;
                        String description = row.description
                                .replace("|", "\\|")
                                .replaceAll("(\\{[a-zA-Z0-9_][a-zA-Z0-9_-]+})", "\\\\$1");
                        overviewWriter.write(String.format("|%s\n|%s\n|%s\n",
                                ...
                                String.format("label:%s[]", row.type),
                                String.format("label:apoc-%s[]", releaseType)));
                        Writer fullCoreWriter = isFull ? fullWriter : coreWriter;

                        fullCoreWriter.write(String.format("|%s\n|%s\n",
                                String.format("%s[%s icon:book[]]\n\n%s", "xref::" + baseUrl + "overview/" + topLevelNamespace + "/" + row.name + ".adoc", row.name, description),
                                String.format("label:%s[]", row.type)));
            ...
            fullWriter.write(footer());
            coreWriter.write(footer());
        } catch (Exception e) {
            throw new RuntimeException(e.getMessage(), e);
        }
    }
    ...
    @NotNull
    private String coreFullHeader(String type) {
        return "[.list-for-apoc-" + type + ", opts=header, cols='5a,1a']\n" +
                "|===\n" +
                "| Qualified Name | Type\n";
    }
