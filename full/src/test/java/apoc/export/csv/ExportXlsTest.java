...
public class ExportXlsTest {
    ...
    public static void setUp() throws Exception {
        ...
        db.executeTransactionally(
                "CREATE (f:User1:User {name:'foo',age:42,male:true,kids:['a','b','c'],location:point({longitude: 11.8064153, latitude: 48.1716114}),dob:date({ year:1984, month:10, day:11 }), created: datetime()})-[:KNOWS]->(b:User {name:'bar',age:42}),(c:User {age:12})");
        db.executeTransactionally(
    ...
    public void testExportAllXls() throws Exception {
        ...
        TestUtil.testCall(
                db,
                "CALL apoc.export.xls.all($file,null)",
                map("file", fileName),
    ...
    public void testExportAllXlsWithCompression() {
        ...
        TestUtil.testResult(
                db,
                "CALL apoc.load.xls($file, 'Address', $config)",
    ...
    public void testExportGraphXls() {
        ...
        TestUtil.testCall(
                db,
                "CALL apoc.graph.fromDB('test',{}) yield graph " + "CALL apoc.export.xls.graph(graph, $file,null) "
    ...
    public void testExportGraphXlsWithCustomHeaderAndMoreThan100Nodes() {
        ...
        TestUtil.testCall(
                db,
                "CALL apoc.graph.fromDB('test',{}) yield graph " + "CALL apoc.export.xls.graph(graph, $file, $conf) "
                        + "YIELD nodes, relationships, properties, file, source,format, time "
                        + "RETURN *",
                map(
                        "file",
    ...
    public void testExportQueryXls() throws Exception {
        ...
        TestUtil.testCall(
                db, "CALL apoc.export.xls.query($query,$file,null)", map("file", fileName, "query", query), (r) -> {
                    assertTrue(
    ...
    public void testExportQueryXlsWithSameLabelAndRelName() {
        db.executeTransactionally(
                "CREATE (u:User{name: 'Andrea'})-[r:COMPANY{since: 2018}]->(c:Company{name: 'Larus'})");
        String fileName = "query.xlsx";
        TestUtil.testCall(
                db,
                "CALL apoc.graph.fromCypher($query, {}, '', {}) YIELD graph AS exportedGraph "
                ...
                map(
                        ...
                        fileName,
                        "exportConf",
                        map("prefixSheetWithEntityType", true),
                        "query",
                        "MATCH (u:User{name: 'Andrea'})-[r:COMPANY]->(c:Company{name: 'Larus'}) return *"),
                (r) -> {
                    assertTrue(
                            "Should get statement", r.get("source").toString().equals("graph: nodes(2), rels(1)"));
                    assertEquals(fileName, r.get("file"));
                    assertEquals("xls", r.get("format"));
                });
    ...
    public void testExportQueryXlsWithJoinedLabels() {
        db.executeTransactionally(
                "CREATE (u:User:Customer{name: 'Andrea'})-[r:COMPANY{since: 2018}]->(c:Company:Customer{name: 'Larus'})");
        String fileName = "query.xlsx";
        TestUtil.testCall(
                db,
                "CALL apoc.graph.fromCypher($query, {}, '', {}) YIELD graph AS exportedGraph "
                ...
                map(
                        ...
                        fileName,
                        "exportConf",
                        map("prefixSheetWithEntityType", true, "joinLabels", true),
                        "query",
                        "MATCH (u:User{name: 'Andrea'})-[r:COMPANY]->(c:Company{name: 'Larus'}) return *"),
                (r) -> {
                    assertTrue(
    ...
    private void assertResults(
        ...
        assertEquals(
                source + String.format(": nodes(%s), rels(%s)", expectedNodesSource, expectedRels), r.get("source"));
        assertEquals(fileName, r.get("file"));
        assertEquals("xls", r.get("format"));
    ...
    private void assertExcelFileForGraph(String fileName, CompressionAlgo algo) {
        assertExcelFileForGraph(
                fileName, "<nodeId>", List.of("<relationshipId>", "<startNodeId>", "<endNodeId>"), algo);
    }

    private void assertExcelFileForGraph(
            String fileName, String headerNode, List<String> headerRel, CompressionAlgo algo) {
        try (InputStream fileInputStream = Files.newInputStream(new File(directory, fileName).toPath());
            ...
            assertEquals(
                    Iterables.count(tx.getAllLabelsInUse()) + Iterables.count(tx.getAllRelationshipTypesInUse()),
                    numberOfSheets);
            ...
            for (Label label : tx.getAllLabelsInUse()) {
                ...
                final Set<String> actual = Iterators.stream(sheet.getRow(0).cellIterator())
                        .map(Cell::getStringCellValue)
                        .collect(Collectors.toSet());
                final Set<String> expected = StreamSupport.stream(
                                tx.getAllNodes().spliterator(), false)
            ...
            for (RelationshipType relType : tx.getAllRelationshipTypesInUse()) {
                long numberOfRels = tx.getAllRelationships().stream()
                ...
                final Set<String> actual = Iterators.stream(sheet.getRow(0).cellIterator())
                        ...
                        .collect(Collectors.toSet());
                final Set<String> expected = StreamSupport.stream(
                                tx.getAllRelationships().spliterator(), false)
                        .filter(rel -> rel.isType(relType))
                        .flatMap(i -> StreamSupport.stream(i.getPropertyKeys().spliterator(), false))
                        .collect(Collectors.toSet());
