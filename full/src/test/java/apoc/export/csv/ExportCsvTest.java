...
public class ExportCsvTest {
    ...
    private void assertHdfsFile(CompressionAlgo compression) {
        ...
        String hdfsUrl = String.format(
                "%s/user/%s/all.csv", miniDFSCluster.getURI().toString(), System.getProperty("user.name"));
        TestUtil.testCall(
                db,
                ...
                (r) -> {
                    try {
                        ...
                        FSDataInputStream inputStream = fs.open(
                                new Path(String.format("/user/%s/all.csv" + fileExt, System.getProperty("user.name"))));
                        ...
                        final String actual =
                                BinaryTestUtil.readFileToString(output, StandardCharsets.UTF_8, compression);
