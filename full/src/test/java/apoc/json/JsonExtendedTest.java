...
public class JsonExtendedTest {
    ...
    public void testJsonValidate() {
        testResult(
                db,
                "CALL apoc.json.validate('{\"osvaldo\": [1,2,3],\n" + "  \"foo\":[\"1\", {\n"
                ...
                (r) -> {
                    ...
                    assertTrue(value.stream()
                            .allMatch(err -> (err.startsWith(ERR_PREFIX + "baz") || err.startsWith(ERR_PREFIX + "bar"))
    ...
    public void testFailOnErrorGetJsonProperty() {
        db.executeTransactionally(
                "CREATE (n:JsonInvalidNode {prop: '{\"foo\": [{\"baz\": 18446744062065078016838}],\"baz\": 7}'})");
    ...
    private void failJsonPropCommon(String query) {
        ...
        failJsonCommon(
                "MATCH (n:JsonInvalidNode) RETURN " + query,
    ...
    private void failJsonStringCommon(String query) {
        ...
        final String call = "CALL apoc.json.validate($json) YIELD value " + "RETURN CASE collect(value) WHEN [] THEN "
                + query + " ELSE 0 END AS value";
    ...
    private void failJsonCommon(String call, String call1, String call2, String call3) {
        ...
        testCall(db, call1, map("json", INVALID_JSON), (row) -> assertEquals(0L, row.get("value")));

        // valid json
        final Object json = TestUtil.singleResultFirstColumn(db, call2, map("json", VALID_JSON));

        // should return an empty list with apoc.json.validate
        testCall(db, call3, map("json", VALID_JSON), (row) -> {
