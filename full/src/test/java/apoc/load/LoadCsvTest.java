...
public class LoadCsvTest {
    ...
    public DbmsRule db = new ImpermanentDbmsRule()
                ...
                .withSetting(GraphDatabaseSettings.load_csv_file_url_root, Paths.get(TestUtil.getUrlFileName("test.csv").toURI()).getParent());
    ...
    public void testLoadCsvWithBinary() {
        testResult(db, "CALL apoc.load.csvParams($file, null, null, $conf)",
                map("file", fileToBinary(new File(getUrlFileName("test.csv")), CompressionAlgo.DEFLATE.name()),
...
RETURN m.col_1,m.col_2,m.col_3
    ...
    @Test public void testWithEmptyQuoteChar() throws Exception {
        ...
        testResult(db, "CALL apoc.load.csv($url, {quoteChar: '\0'})", map("url",url),
