...
public class LoadDirectoryTest {
    ...
    public void testWithNullUrlDir() {
        TestUtil.testFail(
                db,
                "CALL apoc.load.directory('*', null, {recursive: false}) YIELD value RETURN value",
    ...
    public void testAddListenerWithApocLoadDirectoryNotEnabled() {
        ...
        try {
            db.executeTransactionally(
                    "CALL apoc.load.directory.async.add('test','CREATE (n:Test)', '*.csv', '', {}) YIELD name RETURN name");
    ...
    public void testNotAllowSchemaOperation() {
        try {
            db.executeTransactionally(
                    "CALL apoc.load.directory.async.add('test','CREATE INDEX FOR (a:Test) ON (a.name)', '*.csv', '', {}) YIELD name RETURN name");
        } catch (QueryExecutionException e) {
            Throwable except = ExceptionUtils.getRootCause(e);
            TestCase.assertTrue(except instanceof RuntimeException);
    ...
    public void testInvalidQuery() {
        try {
            db.executeTransactionally(
                    "CALL apoc.load.directory.async.add('test','MATCH (n:Test) return invalid', '*.csv', '', {}) YIELD name RETURN name");
        } catch (QueryExecutionException e) {
            Throwable except = ExceptionUtils.getRootCause(e);
            TestCase.assertTrue(except instanceof RuntimeException);
    ...
    public void testAddFolderListenerWithLoadCsv() throws IOException {
        ...
        testCall(
                db,
                "CALL apoc.load.directory.async.add('testRelativePath','" + innerQuery + "', '*.csv', '" + SUBFOLDER_1
                        + "')",
        ...
        final String queryCount = "MATCH (n:CsvToNode {fileName: '" + fileName + "', fileDirectory: '" + SUBFOLDER_1
                + "', listenEventType: 'CREATE', content: ['Selma', '8']}), " + "(m:CsvToNode {fileName: '"
                + fileName + "', fileDirectory: '" + SUBFOLDER_1
                + "', listenEventType: 'CREATE', content: ['Rana', '11']}), " + "(l:CsvToNode {fileName: '"
                + fileName + "', fileDirectory: '" + SUBFOLDER_1
                + "', listenEventType: 'CREATE', content: ['Selina', '18']}) "
                + "RETURN count(n) + count(m) + count(l) AS count";
        // we check that the load csv is not triggered yet
        testResult(
                db,
                queryCount,
                result -> assertEquals(0L, result.columnAs("count").next()));

        final File csvDestination =
                temporaryFolder.newFile(IMPORT_DIR + File.separator + SUBFOLDER_1 + File.separator + fileName);
        csvFile.renameTo(csvDestination);

        assertEventually(
                () -> db.executeTransactionally(
                        queryCount, emptyMap(), (r) -> r.<Long>columnAs("count").next()),
                value -> value == 3L,
                20L,
                TimeUnit.SECONDS);
    ...
    public void testAddFolderListenerWithAllEventTypes() throws IOException {
        db.executeTransactionally(
                "CALL apoc.load.directory.async.add('testAllEvents','CREATE (n:TestEntry)','*.csv') YIELD name RETURN name");
    ...
    private void assertIsRunning(String name) throws Error {
        assertEventually(
                () -> db.executeTransactionally(
                        "CALL apoc.load.directory.async.list() YIELD name, status " + "WHERE name = $name "
                                + "RETURN *",
                        Map.of("name", name),
                        (r) -> {
    ...
    public void testAddFolderListenerWithCreateEventType() throws InterruptedException, IOException {
        db.executeTransactionally(
                "CALL apoc.load.directory.async.add('testSpecific','CREATE (n:TestEntry {fileName: $fileName})', '*.csv', '', {listenEventType: ['CREATE']}) YIELD name RETURN name");
        ...
        assertEventually(
                () -> db.executeTransactionally(
                        queryCount, Map.of("fileName", fileOne), (r) -> r.<Long>columnAs("count")
                                .next()),
                value -> value == 1L,
                30L,
                TimeUnit.SECONDS);
        ...
        testResult(
                ...
                queryCount,
                Map.of("fileName", fileOne),
                result -> assertEquals(1L, result.columnAs("count").next()));
        ...
        assertEventually(
                () -> db.executeTransactionally(
                        queryCount, Map.of("fileName", fileTwo), (r) -> r.<Long>columnAs("count")
                                .next()),
                value -> value == 1L,
                30L,
                TimeUnit.SECONDS);
    ...
    public void testRemoveListenerWithNotExistingName() {
        try {
            System.out.println(db.executeTransactionally(
                    "CALL apoc.load.directory.async.remove('notExisting')", Map.of(), Result::resultAsString));
    ...
    public void testFolderListenerList() {
        db.executeTransactionally(
                "CALL apoc.load.directory.async.add('testOne','CREATE (n:Test)','*.csv','', {}) YIELD name RETURN name");
        db.executeTransactionally(
                "CALL apoc.load.directory.async.add('testTwo','CREATE (n:Test)', '*.json', '', {}) YIELD name RETURN name");
    ...
    public void testFolderListenerNotMatchingPattern() throws IOException, InterruptedException {
        ...
        testCall(
                db,
                "CALL apoc.load.directory.async.add('test','CREATE (n:Test {file: $fileName})','*.json')",
                result -> {
        ...
        db.executeTransactionally(
                "CALL apoc.load.directory.async.add('test','CREATE (n:Test {file: $fileName})','*.json') YIELD name RETURN name");
        Thread.sleep(1000);
    ...
    public void testFolderListenerWithSameName() throws IOException, InterruptedException {
        testCall(
                db,
                "CALL apoc.load.directory.async.add('sameName','CREATE (n:Test {event: $listenEventType, url: $fileDirectory})','*csv','', {interval: 30000}) YIELD name RETURN name",
                r -> assertEquals("sameName", r.get("name")));

        testCall(
                db,
                "CALL apoc.load.directory.async.add('sameName','CREATE (n:TestTwo {event: $listenEventType, url: $fileDirectory, file: $fileName })','*csv','', {}) YIELD name RETURN name",
                r -> assertEquals("sameName", r.get("name")));
        Thread.sleep(1000);
        ...
        final Map<String, Object> queryParams = Map.of(
                ...
                "newFile.csv",
                "url",
                temporaryFolder.getRoot().getPath() + File.separator + IMPORT_DIR,
                "event",
                "CREATE");
        ...
        assertEventually(
                () -> db.executeTransactionally(queryCount, queryParams, (r) -> r.<Long>columnAs("count")
                        .next()),
                value -> value == 1L,
                20L,
                TimeUnit.SECONDS);
        ...
        testResult(
                db,
                "MATCH (n:TestTwo) RETURN count(n) AS count",
                result -> assertEquals(1L, result.columnAs("count").next()));
        testResult(
                db,
                "MATCH (n:Test) RETURN count(n) AS count",
    ...
    public void testAddFolderWithOnlyNameAndCypher() throws InterruptedException, IOException {
        testResult(
                db,
                "CALL apoc.load.directory.async.add('testOnlyNameAndCypher','CREATE (n:TestOnly {prop: $filePath})')",
                result -> {
        ...
        assertEventually(
                () -> db.executeTransactionally(queryCount, Map.of("file", fileExe), (r) -> r.<Long>columnAs("count")
                        .next()),
                value -> value == 1L,
                20L,
                TimeUnit.SECONDS);
    ...
    public void testAddFolderWithRelativePath() throws InterruptedException, IOException {
        testCall(
                db,
                "CALL apoc.load.directory.async.add('testRelativePath','CREATE (n:TestRelativePath)', '*', '"
                        + SUBFOLDER_1 + "')",
                r -> assertEquals("testRelativePath", r.get("name")));
    ...
    public void testAddFolderMultipleListener() throws IOException {
        db.executeTransactionally(
                "CALL apoc.load.directory.async.add('testOne','CREATE (n:TestOne)', '*.csv', '', {}) YIELD name RETURN name");
        assertIsRunning("testOne");
        db.executeTransactionally(
                "CALL apoc.load.directory.async.add('testTwo','CREATE (n:TestTwo)', '*.json', '', {}) YIELD name RETURN name");
        assertIsRunning("testTwo");
        ...
        assertEventually(
                () -> db.executeTransactionally(queryCountLabelOne, emptyMap(), (r) -> r.<Long>columnAs("count")
                        .next()),
                value -> value == 1L,
                30L,
                TimeUnit.SECONDS);
        ...
        assertEventually(
                () -> db.executeTransactionally(queryCountLabelTwo, emptyMap(), (r) -> r.<Long>columnAs("count")
                        .next()),
                value -> value == 1L,
                30L,
                TimeUnit.SECONDS);
    ...
    public void testAddLoadFolderWithFileProtocol() throws InterruptedException, IOException {
        ...
        testResult(
                db,
                "CALL apoc.load.directory.async.add('testExternalUrl','CREATE (n:TestExternalUrl {prop: $fileName})','*.csv','"
                        + folderAsExternalUrl + "', {}) YIELD name RETURN name",
                result -> {
                    Map<String, Object> row = result.next();
    ...
    public void testListenerItemWithError() throws Exception, IOException {
        ...
        db.executeTransactionally(
                "CALL apoc.load.directory.async.add('notExistent', 'CREATE (n:Node)', '*', 'pathNotExistent')");

        assertEventually(
                () -> db.executeTransactionally("CALL apoc.load.directory.async.list()", emptyMap(), (r) -> {
                    Map<String, Object> result = r.next();
    ...
    public void testWithSubfolder() {
        testResult(
                db,
                "CALL apoc.load.directory('*', '" + SUBFOLDER_1 + "', {recursive: false}) YIELD value RETURN value",
                result -> {
    ...
    public void testWithFileProtocolAndRecursiveFalse() {
        ...
        testResult(
                db,
                "CALL apoc.load.directory('*', '" + folderAsExternalUrl
                        + "', {recursive: false}) YIELD value RETURN value",
                result -> {
                    List<Map<String, Object>> rows = Iterators.asList(result.columnAs("value"));
    ...
    public void testWithFileProtocolFilterXlsAndRecursiveTrue() {
        ...
        testResult(
                db,
                "CALL apoc.load.directory('*.xls', '" + folderAsExternalUrl
                        + "', {recursive: true}) YIELD value RETURN value",
                result -> {
                    List<Map<String, Object>> rows = Iterators.asList(result.columnAs("value"));
                    assertTrue(rows.contains(rootTempFolder + File.separator + "Baz.xls"));
    ...
    public void testLoadDirectoryConcatenatedWithLoadCsv() throws URISyntaxException {
        apocConfig().setProperty(APOC_IMPORT_FILE_USE_NEO4J_CONFIG, false);
        File rootTempFolder =
                Paths.get(getUrlFileName("test.csv").toURI()).getParent().toFile();
        String folderAsExternalUrl = "file://" + rootTempFolder;
        testResult(
                db,
                "CALL apoc.load.directory('*.csv', '" + folderAsExternalUrl + "') YIELD value "
                        + "WITH value as url WHERE url ENDS WITH 'test.csv' OR url ENDS WITH 'test-pipe-column.csv' WITH url ORDER BY url DESC CALL apoc.load.csv(url, {results:['map']}) YIELD map RETURN map",
                result -> {
                    Map<String, Object> firstRowFirstFile =
                            (Map<String, Object>) result.next().get("map");
                    assertEquals(Map.of("name", "Selma", "age", "8"), firstRowFirstFile);
                    Map<String, Object> secondRowFirstFile =
                            (Map<String, Object>) result.next().get("map");
                    assertEquals(Map.of("name", "Rana", "age", "11"), secondRowFirstFile);
                    Map<String, Object> thirdRowFirstFile =
                            (Map<String, Object>) result.next().get("map");
                    assertEquals(Map.of("name", "Selina", "age", "18"), thirdRowFirstFile);
                    Map<String, Object> firstRowSecondFile =
                            (Map<String, Object>) result.next().get("map");
                    assertEquals(Map.of("name", "Selma", "beverage", "Soda"), firstRowSecondFile);
                    Map<String, Object> secondRowSecondFile =
                            (Map<String, Object>) result.next().get("map");
                    assertEquals(Map.of("name", "Rana", "beverage", "Tea|Milk"), secondRowSecondFile);
                    Map<String, Object> thirdRowSecondFile =
                            (Map<String, Object>) result.next().get("map");
    ...
    private void restartDb() {
        databaseManagementService.shutdown();
        databaseManagementService = new TestDatabaseManagementServiceBuilder(importFolder.toPath())
                .setConfig(apoc_jobs_pool_num_threads, 40L)
                .build();
        db = databaseManagementService.database(GraphDatabaseSettings.DEFAULT_DATABASE_NAME);
    ...
    public void testWithLimit() throws Exception {
        ...
        long count = TestUtil.singleResultFirstColumn(
                db, "CALL apoc.load.directory.async.list() YIELD name RETURN count(name)");
    ...
    private void createMultipleListeners(int expected) {
        for (int i = 0; i < expected; i++) {
            ...
            db.executeTransactionally(
                    "CALL apoc.load.directory.async.add($nameListener,'CREATE (n:TestOne)', '*.csv', '', {}) YIELD name RETURN name",
