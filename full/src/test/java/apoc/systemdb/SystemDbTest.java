...
public class SystemDbTest {
    ...
    public void testGetGraph() throws Exception {
        TestUtil.testResult(db, "CALL apoc.systemdb.graph() YIELD nodes, relationships RETURN nodes, relationships", result -> {
            ...
            assertEquals(Set.of("TARGETS"), relationships.stream().map(Relationship::getType).map(RelationshipType::name).collect(Collectors.toSet()));
        });
    }
    ...
    public void testExecute() {
        TestUtil.testResult(db, "CALL apoc.systemdb.execute('SHOW DATABASES') YIELD row RETURN row", result -> {
            ...
            org.hamcrest.MatcherAssert.assertThat(rows, Matchers.containsInAnyOrder(
                    MapUtil.map("name", "system", "default", false, "home", false, "currentStatus", "online", "role", "standalone", "requestedStatus", "online", "error", "", "address", "localhost:7687", "aliases", List.of(), "access", "read-write"),
                    MapUtil.map("name", "neo4j", "default", true, "home", true, "currentStatus", "online", "role", "standalone", "requestedStatus", "online", "error", "", "address", "localhost:7687", "aliases", List.of(), "access", "read-write")
            ));
        });
    }
    ...
    public void testExportMetadata() {
        ...
        final String constraintForUuid = "CREATE CONSTRAINT IF NOT EXISTS FOR (n:Person) REQUIRE n.alpha IS UNIQUE;";
