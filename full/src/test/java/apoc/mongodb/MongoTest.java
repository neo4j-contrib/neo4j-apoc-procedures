...
public class MongoTest extends MongoTestBase {
    ...
    static void beforeClassCommon(MongoVersion mongoVersion) throws Exception {
        ...
        try (MongoClient mongoClient = MongoClients.create(format)) {
            ...
            personCollection.insertOne(new Document(map(
                    ...
                    new Symbol("x"),
                    "codeWithScope",
                    new CodeWithScope("function() {}", new Document("k", "v")),
                    "int64",
                    new BsonInt64(29L),
                    "int32",
                    new BsonInt32(29),
    ...
    public void shouldFailIfUriHasNonRootUserNotAuthorizedInDb() {
        try {
            testCall(
                    db,
                    "CALL apoc.mongo.find($uri,{query: {age: 54}})",
                    map(
                            "uri",
                            String.format(
                                    "mongodb://user:secret@%s:%s/foo.bar?authSource=admin",
                                    mongo.getContainerIpAddress(), mongo.getMappedPort(MONGO_DEFAULT_PORT))),
                    row -> {});
        } catch (Exception e) {
    ...
    public void shouldNotFailIfUriHasRootUser() {
        testCall(
                db,
                "CALL apoc.mongo.find($uri, {age: 54})",
                map(
                        "uri",
                        String.format(
                                "mongodb://admin:pass@%s:%s/foo.bar?authSource=admin",
                                mongo.getContainerIpAddress(), mongo.getMappedPort(MONGO_DEFAULT_PORT))),
                row -> {
    ...
    public void shouldFailIfUriHasNotCredentials() {
        try {
            testCall(
                    db,
                    "CALL apoc.mongo.find($uri, {query: {foo: 'bar' }})",
                    map(
                            "uri",
                            String.format(
                                    "mongodb://%s:%s/test.test?authSource=admin",
                                    mongo.getContainerIpAddress(), mongo.getMappedPort(MONGO_DEFAULT_PORT))),
                    row -> {});
        } catch (Exception e) {
    ...
    public void shouldFailIfUriHasNotDatabase() {
        try {
            testCall(
                    db,
                    "CALL apoc.mongo.find($uri,{name:'testDocument'})",
                    map(
                            "uri",
                            String.format(
                                    "mongodb://admin:pass@%s:%s/?authSource=admin",
                                    mongo.getContainerIpAddress(), mongo.getMappedPort(MONGO_DEFAULT_PORT))),
                    r -> {});
        } catch (Exception e) {
    ...
    public void shouldFailIfNeitherUriNorConfigHasCollection() {
        try {
            testCall(
                    db,
                    "CALL apoc.mongo.count($uri,{name:'testDocument'})",
                    map(
                            "uri",
                            String.format(
                                    "mongodb://admin:pass@%s:%s/test?authSource=admin",
                                    mongo.getContainerIpAddress(), mongo.getMappedPort(MONGO_DEFAULT_PORT))),
                    r -> {});
        } catch (Exception e) {
    ...
    public void shouldNotFailsIfUriHasNotCollectionNameButIsPresentInConfig() {
        testCall(
                db,
                "CALL apoc.mongo.count($uri, {name:'testDocument'}, {collection: 'test'})",
                map(
                        "uri",
                        String.format(
                                "mongodb://admin:pass@%s:%s/test?authSource=admin",
    ...
    public void objectIdAsString() {
        ...
        testResult(
                db,
                "CALL apoc.mongo.find($uri, {binary: {`$binary`: $bytes, `$subType`: '00'}}, {objectIdAsMap: false})",
    ...
    public void testWithSkip() {
        testResult(
                db,
                "CALL apoc.mongo.find($uri, {expr: {`$regex`: 'foo*', `$options`: ''}}, {skip: 1})",
                map("uri", PERSON_URI),
                res -> {
                    final ResourceIterator<Map<String, Object>> values = res.columnAs("value");
    ...
    public void testWithLimit() {
        testResult(
                db,
                "CALL apoc.mongo.find($uri, {expr: {`$regex`: 'foo*', `$options`: ''}}, {limit: 2})",
                map("uri", PERSON_URI),
                res -> {
                    final ResourceIterator<Map<String, Object>> values = res.columnAs("value");
    ...
    public void testWithSkipAndLimit() {
        testResult(
                db,
                "CALL apoc.mongo.find($uri, {expr: {`$regex`: 'foo*', `$options`: ''}}, {skip: 1, limit: 1})",
                map("uri", PERSON_URI),
                res -> {
    ...
    public void testAggregation() {
        testResult(
                db,
                "CALL apoc.mongo.aggregate($uri, [{`$match`: {foo: 'custom'}}, {`$sort`: {name: -1}}, {`$skip`: 1}, {`$limit`: 2}, {`$set`: {aggrField: 'Y'} }], $conf)",
                map("uri", PERSON_URI, "conf", map("objectIdAsMap", false)),
                res -> {
                    final ResourceIterator<Map<String, Object>> value = res.columnAs("value");
    ...
    public void testFindSort() {
        testResult(
                db,
                "CALL apoc.mongo.find($uri, {expr: {`$regex`: 'foo*', `$options`: ''}}, {sort: {name: -1} })",
                map("uri", PERSON_URI),
                res -> {
                    final ResourceIterator<Map<String, Object>> value = res.columnAs("value");
                    assertionsPersonJohn(value.next(), true, false);
    ...
    public void testFindWithExtractReference() {
        testResult(
                db,
                "CALL apoc.mongo.find($uri, {expr: {`$regex`: 'foo*', `$options`: ''}}, {extractReferences: true})",
                map("uri", PERSON_URI),
                res -> {
                    final ResourceIterator<Map<String, Object>> value = res.columnAs("value");
    ...
    public void testFindWithProject() {
        testResult(
                db,
                "CALL apoc.mongo.find($uri, {expr: {`$regex`: 'foo*', `$options`: ''}}, {project: {age: 1}  })",
                map("uri", PERSON_URI),
                res -> {
                    final ResourceIterator<Map<String, Object>> value = res.columnAs("value");
    ...
    public void testFindWithManyConfigs() {
        testResult(
                db,
                "CALL apoc.mongo.find($uri, {baz: 'baa'}, {project: {age: 1}, sort: {name: -1}, objectIdAsMap: false, skip: 1, limit: 3})",
                map("uri", PERSON_URI),
                this::assertionsWithManyConfig);
    }
    ...
    public void testFindWithExtractReferencesTrue() {
        testCall(
                db,
                "CALL apoc.mongo.find($uri, {`_id`: {`$oid`: '97e193d7a9cc81b4027519b4'}}, {extractReferences: true, objectIdAsMap: false})",
                map("uri", PERSON_URI),
                r -> {
    ...
    public void testUpdate() {
        testCall(
                db,
                "CALL apoc.mongo.update($uri, {foo: {`$oid`: '57e193d7a9cc81b4027499c4'}}, {`$set`:{code: {`$code`: 'void 0'}}})",
                map("uri", PERSON_URI),
                r -> {
        ...
        testCall(
                db,
                "CALL apoc.mongo.update($uri, {foo: {`$oid`: '57e193d7a9cc81b4027499c4'}},{`$set`:{code: {`$code`: 'function() {}'}}})",
                map("uri", PERSON_URI),
                r -> {
                    long affected = (long) r.get("value");
                    assertEquals(1, affected);
    ...
    public void testInsert() {
        testResult(
                db,
                "CALL apoc.mongo.insert($uri, [{secondId: {`$oid`: '507f191e811c19729de860ea'}, baz: 1}, {secondId: {`$oid`: '507f191e821c19729de860ef'}, baz: 1}])",
                map("uri", PERSON_URI),
    ...
    public void testInsertWithRegex() {
        testResult(
                db,
                "CALL apoc.mongo.insert($uri,[{foo:{`$regex`: 'pattern', `$options`: ''}, myId: {`$oid`: '507f191e811c19729de960ea'}}])",
                map("uri", PERSON_URI),
                (r) -> assertFalse("should be empty", r.hasNext()));

        testCall(
                db,
                "CALL apoc.mongo.find($uri, {foo: {`$regex`: 'pattern', `$options`: ''}})",
                map("uri", PERSON_URI),
                r -> {
        ...
        testCall(
                db,
                "CALL apoc.mongo.delete($uri, {foo: {`$regex`: 'pattern', `$options`: ''}})",
                map("uri", PERSON_URI),
                r -> {
                    long affected = (long) r.get("value");
    ...
    public void testGetInsertAndDeleteWithObjectId() {
        testResult(
                db,
                "CALL apoc.mongo.insert($uri,[{foo:'bar', myId: {`$oid`: '507f191e811c19729de960ea'}}])",
                map("uri", PERSON_URI),
                (r) -> {
        ...
        testCall(
                db,
                "CALL apoc.mongo.delete($uri,{foo: 'bar', myId: {`$oid`: '507f191e811c19729de960ea'}})",
                map("uri", PERSON_URI),
                r -> {
                    long affected = (long) r.get("value");
                    assertEquals(1L, affected);
        ...
        testResult(
                db,
                "CALL apoc.mongo.find($uri,{query: {foo:'bar', myId: {`$oid`: '507f191e811c19729de960ea'}}})",
                map("uri", PERSON_URI),
                r -> {
                    assertFalse("should be empty", r.hasNext());
                });
    ...
    public void testInsertFailsWithDuplicateKey() {
        try {
            testResult(
                    db,
                    "CALL apoc.mongo.insert($uri, [{foo:'bar', _id: 1}, {foo:'bar', _id: 1}])",
                    map("uri", TEST_URI),
                    (r) -> {
                        assertFalse("should be empty", r.hasNext());
                    });
        ...
        testResult(
                ...
                map(
                        ...
                        map(
                                ...
                                map(
                                        "$",
                                        "Person:Customer{!name,bought,coordinates,born}",
                                        "$.bought",
    ...
    private void assertDocumentIdAndRefs(
            Map<String, Object> value, boolean extractReferences, boolean idAsMap, ObjectId idAlAsObjectId) {
