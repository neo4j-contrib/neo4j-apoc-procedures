...
public class PeriodicExtendedTest {
    ...
    public void testSubmitSchema() {
        testCall(
                db,
                "CALL apoc.periodic.submitSchema('subSchema','CREATE INDEX periodicIdx FOR (n:Bar) ON (n.first_name, n.last_name)')",
                (row) -> {
        ...
        assertEventually(
                () -> db.executeTransactionally(
                        "SHOW INDEXES YIELD name WHERE name = 'periodicIdx' RETURN count(*) AS count",
    ...
    public void testSubmitSchemaWithWriteOperation() {
        testCall(db, "CALL apoc.periodic.submitSchema('subSchema','CREATE (:SchemaLabel)')", (row) -> {
            assertEquals("subSchema", row.get("name"));
            assertEquals(false, row.get("done"));
        });
        ...
        assertEventually(
                () -> db.executeTransactionally(
                        "MATCH (n:SchemaLabel) RETURN count(n) AS count",
                        Collections.emptyMap(),
                        (res) -> res.<Long>columnAs("count").next()),
                val -> val == 1L,
                15L,
    ...
    public void testRebindWithNlpWriteProcedure() {
        // use case:
        // https://community.neo4j.com/t5/neo4j-graph-platform/use-of-apoc-periodic-iterate-with-apoc-nlp-gcp-classify-graph/m-p/56846#M33854
        ...
        final String actionRebind =
                "CALL apoc.nlp.gcp.classify.graph(apoc.node.rebind(node), $nlpConf) YIELD graph RETURN null";
        ...
        final String actionId =
                "MATCH (node) WHERE id(node) = id CALL apoc.nlp.gcp.classify.graph(node, $nlpConf) YIELD graph RETURN null";
    ...
    public void testRebindWithMapIterationAndCreateRelationshipProcedure() {
        ...
        final String action =
                "CREATE (node:Category) with map.key as art, node call apoc.create.relationship(art, 'CATEGORY', {b: 1}, node) yield rel return rel";
        testRebindCommon(iterate, action, 0, this::assertNodeDeletedErr);
    ...
    private void assertNodeDeletedErr(Map<String, Object> r) {
        assertTrue(((Map<String, Object>) r.get("errorMessages"))
                .keySet().stream()
                        .anyMatch(
                                k -> k.matches("Node\\[\\d+] is deleted and cannot be used to create a relationship")));
    }

    private void testRebindCommon(
            String iterate, String action, int expected, Consumer<Map<String, Object>> assertions) {
        final Map<String, Object> nlpConf =
                map("key", "myKey", "nodeProperty", "content", "write", true, "unsupportedDummyClient", true);
        ...
        testCall(
                db,
                "CALL apoc.periodic.iterate($iterate, $action, $config)",
    ...
    public void testRock_n_roll() {
        ...
        testResult(
                db,
                "CALL apoc.periodic.rock_n_roll('match (p:Person) return p', 'WITH $p as p SET p.lastname =p.name REMOVE p.name', 10)",
                result -> {
    ...
    public void testTerminateRockNRoll() {
        PeriodicTestUtils.testTerminatePeriodicQuery(
                db,
                "CALL apoc.periodic.rock_n_roll('UNWIND range(0,1000) as id RETURN id', 'CREATE (:Foo {id: $id})', 10)");
    }
    ...
    public void testIterateErrors() {
        testResult(
                db,
                "CALL apoc.periodic.rock_n_roll('UNWIND range(0,99) as id RETURN id', 'CREATE (:Foo {id: 1 / ($id % 10)})', 10)",
                result -> {
                    Map<String, Object> row = Iterators.single(result);
                    assertEquals(10L, row.get("batches"));
                    assertEquals(100L, row.get("total"));
    ...
    public void testRock_n_roll_while() {
        ...
        testResult(
                db,
                "CALL apoc.periodic.rock_n_roll_while('return coalesce($previous,3)-1 as loop', 'match (p:Person) return p', 'MATCH (p) where p=$p SET p.lastname =p.name', 10)",
                result -> {
                    long l = 0;
    ...
    public void testRockNRollWhileFail() {
        ...
        try {
            ...
            String expected =
                    "Failed to invoke procedure `apoc.periodic.rock_n_roll_while`: Caused by: java.lang.RuntimeException: Exception for field `cypherLoop`, message: Unknown function 'coalescence' (line 1, column 16 (offset: 15))"
                            + newline + "\"EXPLAIN return coalescence($previous, 3) - 1 as loop\""
                            + newline + "                ^\n"
                            ...
                            + newline + "\"EXPLAIN match (p:Person) return pp\""
                            + newline + "                                 ^";
