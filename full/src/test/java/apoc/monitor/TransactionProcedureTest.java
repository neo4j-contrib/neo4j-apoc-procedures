...
public class TransactionProcedureTest {
    ...
    public void testGetStoreInfo() {
        ...
        testCall(db, "CALL apoc.monitor.tx()", (row) -> {
            ...
            assertEquals(1l, lastTxId.addAndGet((long) row.get("lastTxId")));
            assertEquals(1l, row.get("currentOpenedTx"));
            assertEquals(3l, totalOpenedTx.addAndGet((long) row.get("totalOpenedTx")));
            assertEquals(2l, totalTx.addAndGet((long )row.get("totalTx")));
        });
