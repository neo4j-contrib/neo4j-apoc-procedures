...
public class StaticTest {
    ...
    public final ProvideSystemProperty systemPropertyRule =
            new ProvideSystemProperty("apoc.static.test", VALUE).and("apoc.static.all.test", VALUE);
    ...
    public void testGetAllFromConfig() throws Exception {
        TestUtil.testCall(
                db, "return apoc.static.getAll('all') as value", r -> assertEquals(map("test", VALUE), r.get("value")));
        TestUtil.testCall(db, "call apoc.static.set('all.test2',42)", r -> assertNull(r.get("value")));
        TestUtil.testCall(
                db,
                "return apoc.static.getAll('all') as value",
                r -> assertEquals(map("test", VALUE, "test2", 42L), r.get("value")));
        TestUtil.testCall(
                db,
                "return apoc.static.getAll('') as value",
                r -> assertEquals(map("test", VALUE, "all.test", VALUE, "all.test2", 42L), r.get("value")));
    }
