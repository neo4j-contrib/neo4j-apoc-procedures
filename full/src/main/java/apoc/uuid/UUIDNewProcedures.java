...
public class UUIDNewProcedures {
    ...
    public Stream<UuidInfo> setup(
        ...
        // unlike the apoc.uuid.install we don't return the UuidInstallInfo because we don't retrieve the
        // `batchComputationResult` field
    ...
    public Stream<UuidInfo> drop(
            @Name("label") String label, @Name(value = "databaseName", defaultValue = "neo4j") String databaseName) {
        checkInSystemLeader(databaseName);
    ...
    public Stream<UuidInfo> show(@Name(value = "databaseName", defaultValue = "neo4j") String databaseName) {
        ...
        return withSystemDb(sysTx -> {
                    return UUIDHandlerNewProcedures.getUuidNodes(sysTx, databaseName).stream()
                            .map(UuidInfo::new)
