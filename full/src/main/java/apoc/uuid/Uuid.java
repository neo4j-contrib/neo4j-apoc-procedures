...
public class Uuid {
    ...
    public Stream<UuidInstallInfo> install(
            @Name("label") String label, @Name(value = "config", defaultValue = "{}") Map<String, Object> config) {
        ...
        return Stream.of(new UuidInstallInfo(
                label,
                true,
    ...
    public Stream<UuidInfo> removeAll() {
        ...
        return removed.entrySet().stream().map(e -> {
            ...
            return new UuidInfo(
                    e.getKey(),
                    false,
                    Map.of("uuidProperty", conf.getUuidProperty(), "addToSetLabels", conf.isAddToSetLabels()));
        });
    }
    ...
    public Stream<UuidInfo> list() {
        return uuidHandler.list().entrySet().stream().map((e) -> {
            final UuidConfig conf = e.getValue();
            return new UuidInfo(
                    e.getKey(),
                    true,
    ...
    public static class UuidInstallInfo extends UuidInfo {
        ...
        UuidInstallInfo(
                ...
                Map<String, Object> batchComputationResult) {
            super(label, installed, properties);
            this.batchComputationResult = batchComputationResult;
        }
    }

    public static Map<String, Object> setExistingNodes(
            GraphDatabaseService db, Pools pools, String label, UuidConfig uuidConfig) {
        ...
        return Util.inTx(db, pools, txInThread -> {
            ...
            String action = "MATCH (n) WHERE id(n) = id SET n." + Util.sanitizeAndQuote(uuidConfig.getUuidProperty())
                    + " = " + uuidFunctionName + "()";
            return txInThread
                    .execute(
    ...
    public static String getUuidFunctionName() {
        ApocConfig.UuidFormatType formatType = ApocConfig.apocConfig()
                .getEnumProperty(
                        ApocConfig.APOC_UUID_FORMAT, ApocConfig.UuidFormatType.class, ApocConfig.UuidFormatType.hex);
        switch (formatType) {
            case base64:
