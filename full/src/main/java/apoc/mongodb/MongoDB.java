...
public class MongoDB {
    ...
    public Stream<MapResult> get(
        ...
        return executeMongoQuery(
                ...
                e -> log.error(
                        "apoc.mongodb.get - hostOrKey = [" + hostOrKey + "], db = [" + db + "], collection = ["
                                + collection + "], query = [" + query + "], compatibleValues = [" + compatibleValues
    ...
    public Stream<LongResult> count(
        ...
        return executeMongoQuery(
                ...
                e -> log.error(
                        "apoc.mongodb.count - hostOrKey = [" + hostOrKey + "], db = [" + db + "], collection = ["
    ...
    public Stream<MapResult> first(
        ...
        return executeMongoQuery(
                ...
                e -> log.error(
                        "apoc.mongodb.first - hostOrKey = [" + hostOrKey + "], db = [" + db + "], collection = ["
                                + collection + "], query = [" + query + "], compatibleValues = [" + compatibleValues
                                + "]",
                        e));
    }
    ...
    public Stream<MapResult> find(
        ...
        return executeMongoQuery(
                ...
                e -> log.error(
                        "apoc.mongodb.find - hostOrKey = [" + hostOrKey + "], db = [" + db + "], collection = ["
                                + collection + "], query = [" + query + "], project = [" + project + "], sort = ["
                                + sort + "], compatibleValues = [" + compatibleValues + "], skip = [" + skip
                                + "], limit = [" + limit + "]",
                        e));
    }
    ...
    public Stream<LongResult> delete(
        ...
        return executeMongoQuery(
                ...
                e -> log.error(
                        "apoc.mongodb.delete - hostOrKey = [" + hostOrKey + "], db = [" + db + "], collection = ["
                                + collection + "], query = [" + query + "]",
                        e));
    }
    ...
    public Stream<LongResult> update(
            ...
            @Name("collection") String collection,
            @Name("query") Map<String, Object> query,
            @Name("update") Map<String, Object> update) {
        return executeMongoQuery(
                hostOrKey,
                db,
                collection,
                ...
                e -> log.error(
                        "apoc.mongodb.update - hostOrKey = [" + hostOrKey + "], db = [" + db + "], collection = ["
                                + collection + "], query = [" + query + "], update = [" + update + "]",
                        e));
    }
    ...
    public Stream<MapResult> byObjectId(
            @Name("host") String hostOrKey,
            @Name("db") String db,
            @Name("collection") String collection,
        ...
        return executeMongoQuery(
                ...
                db,
                collection,
                conf.isCompatibleValues(),
                conf.isExtractReferences(),
                conf.isObjectIdAsMap(),
                coll -> {
                    Map<String, Object> result = coll.first(Map.of(conf.getIdFieldName(), new ObjectId(objectIdValue)));
                    return result == null || result.isEmpty() ? Stream.empty() : Stream.of(new MapResult(result));
                },
                e -> log.error(
                        "apoc.mongodb.get.byObjectId - hostOrKey = [" + hostOrKey + "], db = [" + db
                                + "], collection = [" + collection + "], objectIdValue = [" + objectIdValue + "]",
                        e));
    }
