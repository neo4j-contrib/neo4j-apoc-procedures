...
public class Weaviate {
    ...
    public Stream<MapResult> createCollection(
        ...
        Map<String, Object> additionalBodies =
                Map.of("class", collection, "vectorIndexConfig", Map.of("distance", similarity, "size", size));
    ...
    private Stream<EmbeddingResult> getCommon(
        ...
        return ids.stream().flatMap(id -> {
            String endpoint =
                    String.format("%s/objects/%s/%s", conf.getApiConfig().getBaseUrl(), collection, id) + suffix;
