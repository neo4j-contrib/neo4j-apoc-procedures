...
You can export your graph as an unweighted network, which results in assigning each relationship a default weight of 1.0. 
...
You can have your weight value stored as a number(integer,float) or a string. If the weight value is not valid or missing, it will use the default 1.0 value.  
