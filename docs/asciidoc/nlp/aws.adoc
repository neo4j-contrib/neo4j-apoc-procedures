...
Amazon Web Services (AWS) https://docs.aws.amazon.com/comprehend/index.html[Comprehend Natural Language API^] lets users derive insights from unstructured text using Google machine learning.
...
[source,cypher]
----
CALL apoc.nlp.aws.entities.stream(sourceNode:Node, {
  key: String,
  secret: String,
  nodeProperty: String
})
...
[source,cypher]
----
CALL apoc.nlp.aws.entities.graph(sourceNode:Node, {
  key: String,
  secret: String,
  nodeProperty: String,
