...
Google Cloud Platform's Natural Language API lets users derive insights from unstructured text using Google machine learning.
...
* <<ai-gcp-classify>>
* <<ai-gcp-examples>>
    ** <<ai-gcp-examples-entities>>
    ** <<ai-gcp-examples-classify>>
...
|===
include::../../../build/generated-documentation/apoc.ai.gcp.entities.csv[]
include::../../../build/generated-documentation/apoc.ai.gcp.classify.csv[lines=2..3]
|===
...
The entity extraction procedures (`apoc.ai.gcp.entities.*`) are wrappers around the https://cloud.google.com/natural-language/docs/reference/rest/v1/documents/analyzeEntities[`documents.analyzeEntities`^] method of the Google Natural Language API.
...
In addition, `apoc.ai.gcp.entities.graph` supports the following config parameters:
...
[source,cypher]
----
CALL apoc.ai.gcp.entities.stream(sourceNode:Node, {
  key: String,
  nodeProperty: String
})
YIELD response
...
[source,cypher]
----
CALL apoc.ai.gcp.entities.graph(sourceNode:Node, {
  key: String,
  nodeProperty: String,
...
The entity extraction procedures (`apoc.ai.gcp.classify.*`) are wrappers around the https://cloud.google.com/natural-language/docs/reference/rest/v1/documents/classifyText[`documents.classifyText`^] method of the Google Natural Language API.
...
[separator=¦,opts=header,cols="1,1m,1m,5"]
|===
include::../../../build/generated-documentation/apoc.ai.gcp.classify.csv[]
|===
...
In addition, `apoc.ai.gcp.classify.graph` supports the following config parameters:
...
[source,cypher]
----
CALL apoc.ai.gcp.classify.stream(sourceNode:Node, {
  key: String,
  nodeProperty: String
})
YIELD response
...
[source,cypher]
----
CALL apoc.ai.gcp.classify.graph(sourceNode:Node, {
  key: String,
  nodeProperty: String,
  relationshipType: String,
  write: Boolean
...
YIELD response
UNWIND response.entities AS entity
RETURN entity;
----
...
We can see a Neo4j Browser visualization of the virtual graph in <<apoc.ai.gcp.entities.graph.svg>>.

[[apoc.ai.gcp.entities.graph.svg]]
image::apoc.ai.gcp.entities.graph.svg[title="Pokemon entities graph"]
...
----
MATCH (a:Article {uri: "https://neo4j.com/blog/pokegraph-gotta-graph-em-all/"})
CALL apoc.ai.gcp.classify.stream(a, {
  key: $apiKey,
  nodeProperty: "body"
})
YIELD response
UNWIND response.categories AS category
RETURN category;
----
...
We can see a Neo4j Browser visualization of the virtual graph in <<apoc.ai.gcp.classify.graph.svg>>.

[[apoc.ai.gcp.classify.graph.svg]]
image::apoc.ai.gcp.classify.graph.svg[title="Pokemon categories graph"]
