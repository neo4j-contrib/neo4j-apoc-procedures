...
The examples in this section are based on a virtual graph that contains all `KNOWS` relationships and the nodes either side of that relationship.
The query below creates a virtual graph and stores it in memory with the name `knows.cached` using <<static-values, Static Value Storage>>.

[source,cypher]
----
MATCH path = (:User)-[:KNOWS]->(:User)
WITH collect(path) AS paths
CALL apoc.graph.fromPaths(paths, "knows", {})
YIELD graph AS g
CALL apoc.static.set("knows.cached", g)
YIELD value
RETURN value, g
----
...
|===
| value | g
| NULL | {name: "knows", relationships: [[:KNOWS {since: 1993}]], nodes: [(:User {born: 2015-07-04T19:32:24, name: "Adam", place: point({srid:4326, x:33.46789, y:13.1}), age: 42, male: TRUE, kids: ["Sam", "Anna", "Grace"]}), (:User {name: "Jim", age: 42})], properties: {}} 
|===
...
[source,cypher]
----
CALL apoc.export.json.graph(apoc.static.get("knows.cached"),"graph.json",{})
YIELD file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data 
RETURN file, source, format, nodes, relationships, properties, time, rows, batchSize, batches, done, data
----
...
|===
| file         | source                        | format | nodes | relationships | properties | time | rows | batchSize | batches | done | data 
|   "graph.json" | "graph: nodes(2), rels(1)" | "json" | 2     | 1             | 9          | 2    | 3    | -1        | 0       | TRUE | NULL
|===
