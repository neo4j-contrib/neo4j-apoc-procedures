...
[abstract]
--
This section describes a procedure that can be used to expand the paths of variable length path traversals.
--

The expand paths procedure is the most basic of the <<path-expander, path expanders>>.
This procedure enables path traversals based on relationship filters and node filters.
See <<path-expander-paths-config>> if more control is required over the traversal.

This section includes the following:

* <<path-expander-paths-procedure-overview, Procedure Overview>>
* <<path-expander-paths-parameter-syntax, Parameter Syntax>>
* <<path-expander-paths-examples, Examples>>
...
|===
include::../../../build/generated-documentation/apoc.path.expand.csv[]
|===

[[path-expander-paths-parameter-syntax]]
=== Parameter Syntax

This procedure takes five parameters:
...
We can also provide a node filter to restrict the nodes that are returned.
The following query only returns paths where every node has the `Engineering` label.

.The following returns paths containing only `Engineering` people that Praveena `KNOWS` from 1 to 3 hops
[source,cypher]
----
MATCH (p:Person {name: "Praveena"})
...
This query returns a lot of paths, Alicia is very well connected!
