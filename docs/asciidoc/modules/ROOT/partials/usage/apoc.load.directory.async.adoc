We can pass from 2 to 5 parameter in the `apoc.load.directory.async.add`.
...
We can remove a specific listener executing the
[source,cypher]
----
CALL apoc.load.directory.async.remove('<LISTENER_NAME>')
----
