...
NOTE: `apoc.custom.installProcedure` has to be executed against the `system` database (see the note above), otherwise it fails with an error such as `No write operations are allowed directly on this database. Writes must pass through the leader.`, even on a single-instance, non-clustered setup.
...
Alternatively, we can pass `null` to install the procedure globally across all databases.
If the procedure already exists locally, it will be overwritten by the global version, e.g.
[source,cypher]
----
:use system
CALL apoc.custom.installProcedure('foodb() :: (row::INT)', 'RETURN 42 as row', null)
----
