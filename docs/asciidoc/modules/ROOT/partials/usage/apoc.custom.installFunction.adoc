...
NOTE: `apoc.custom.installFunction` has to be executed against the `system` database (see the note above), otherwise it fails with an error such as `No write operations are allowed directly on this database. Writes must pass through the leader.`, even on a single-instance, non-clustered setup.
...
Alternatively, we can pass `null` to install the function globally across all databases.
If the function already exists locally, it will be overwritten by the global version, e.g.
[source,cypher]
----
:use system
CALL apoc.custom.installFunction('foodb() :: INT', 'RETURN 42', null)
----
