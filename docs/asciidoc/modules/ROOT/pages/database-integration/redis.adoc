...
¦apoc.redis.dispatch¦apoc.redis.dispatch(uri :: STRING?, command :: STRING?, output :: STRING?, keys = [] :: LIST? OF STRING?, values = [] :: LIST? OF STRING?, arguments = {} :: MAP?, config = {} :: MAP?) :: (value :: ANY?)¦apoc.redis.dispatch(uri, command, output, keys, values, arguments, \{config}) | Execute a custom command based on https://lettuce.io/ BaseRedisCommands.dispatch(...)
...
Download it from https://github.com/lettuce-io/lettuce-core/releases/tag/6.1.1.RELEASE[this url].
...
We can create a custom command using the https://lettuce.io/core/release/reference/[Lettuce Java Driver], 
declaring the https://lettuce.io/lettuce-4/release/api/com/lambdaworks/redis/protocol/CommandType.html[CommandType], 
the https://lettuce.io/core/6.0.0.RELEASE/api/io/lettuce/core/output/CommandOutput.html[CommandOutput],
the key(s), the value(s) and the argument(s).

For example, to create a `LPUSH` command:

[source,cypher]
----
CALL apoc.redis.dispatch($uri, 'LPUSH', 'IntegerOutput', ['key'], ['valueList'])
----
...
[source,cypher]
----
CALL apoc.redis.dispatch($uri, 'MSET', 'BooleanOutput', null, null, {keyOne: 'valueOne', keyTwo: 'valueTwo'})
----
