...
| vectorKey, metadataKey, scoreKey, textKey | used with the `apoc.vectordb.custom.get` procedure.
    ...
    See examples below.
| fields | a list of fields to be returned, it must be specified for `apoc.vectordb.weaviate.query`, `apoc.vectordb.milvus.get` and `apoc.vectordb.milvus.query`.

    The keys present in the `metadataKey`, defined in the `mapping` config, will be added in addition to the ones specified in the `fields` parameter.
    If the `fields` parameter is absent, the procedures will return the fields specified in the `metadataKey` defined in the `mapping` config.

     If both the `fields` and the `metadataKey` parameters are absent or empty, the mentioned procedures will return an error.
|===
