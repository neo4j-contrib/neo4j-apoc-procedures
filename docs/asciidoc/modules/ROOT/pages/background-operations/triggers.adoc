...
[NOTE]
====
All these `apoc.trigger.*` procedures are implemented to be executed in the system database,
therefore they have to be used executing first `:use system` (in cypher-shell or Neo4j Browser), 
by opening a system db session with a neo4j driver, 
or by prepending the suffix `USE system` to the cypher statement.
