...
While during imports you can use `CALL ... IN TRANSACTIONS` to control transaction sizes in memory, but for other graph refactorings it's not that easy to commit transactions regularly to free memory for new update state.
...
To visualize verbose progresses' logs of `apoc.periodic.iterate` or `apoc.periodic.commit` please set `dbms.logs.debug.level=DEBUG` in `neo4j.conf`.
