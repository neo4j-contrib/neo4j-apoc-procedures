...
|===
¦Qualified Name¦Type¦Release
include::example$generated-documentation/apoc.uuid.create.adoc[]
...
[source,cypher]
----
CALL apoc.uuid.create('neo4j', 'Person')
YIELD label, installed, properties
RETURN label, installed, properties
----

Note that the `apoc.uuid.create`, as well as the `apoc.uuid.drop`, `apoc.uuid.dropAll`,
...
[source,cypher]
----
CALL apoc.uuid.create('neo4j', 'TestShow')
----
...
use the xref::overview/apoc.uuid/apoc.uuid.list.adoc[apoc.uuid.list] against the session database ("neo4j" in the above case).
====
...
[source,cypher]
----
CALL apoc.uuid.drop('neo4j', 'Person')
YIELD label, installed, properties
RETURN label, installed, properties
----
