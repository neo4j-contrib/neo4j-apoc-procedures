...
[source,cypher]
----
CREATE CONSTRAINT FOR (bar:Bar) REQUIRE exists(bar.foobar)
...
[source,cypher]
----
CREATE CONSTRAINT FOR ()-[like:LIKED]-() REQUIRE exists(like.day)
CREATE CONSTRAINT FOR ()-[starred:STARRED]-() REQUIRE exists(starred.month)
----
