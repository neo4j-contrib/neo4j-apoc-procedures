[[load-csv]]
= Load CSV
:page-custom-canonical: https://neo4j.com/labs/apoc/5/import/load-csv/
...
From 4.3.0.10 onwards, for 4.3 versions, the behaviour is to escape a delimiter and then split the columns. For this input:
...
we get an error containing the following message:
