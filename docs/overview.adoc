...
| CALL apoc.json.getJsonProperty(node,key) | converts serialized JSON in property back to original object
| CALL apoc.json.getJsonPropertyMap(node,key) | converts serialized JSON in property back to map
...
[cols="1m,5"]
|===
| CALL apoc.text.replace(text, regex, replacement) YIELD value | replace each substring of the given string that matches the given regular expression with the given replacement.
...
| apoc.date.systemTimezone() | return the system timezone default name
