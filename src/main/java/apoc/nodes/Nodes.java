...
//            count += Util.inTx(api,() -> batch.stream().peek( n -> {n.getRelationships().forEach(Relationship::delete);n.delete();}).count());
    ...
    static class Degree implements Comparable<Degree> {
        ...
        public boolean isConnected(ReadOperations ops, MatchingRelationshipVisitor matcher) throws EntityNotFoundException {
            ...
            matcher.reset();
            if (direction == Direction.OUTGOING) return checkRelationships(ops.nodeGetRelationships(node, Direction.OUTGOING, type), matcher);
            if (direction == Direction.INCOMING) return checkRelationships(ops.nodeGetRelationships(node, Direction.INCOMING, type), matcher);
            return checkRelationships(ops.nodeGetRelationships(node, Direction.BOTH, type), matcher);
        }
    }
